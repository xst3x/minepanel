    ...
    return (accept) => {
        ...
        sftp.on('OPEN', (reqId, filename, flags, attrs) => {
            ...
            fs.open(fpath, fsFlags, attrs.mode || 0o644, (err, fd) => {
                if (err)
                    return sftp.status(reqId, statusCode(err));
        ...
        sftp.on('READ', (reqId, handle, offset, length) => {
            ...
            fs.read(obj.fd, buf, 0, length, offset, (err, bytesRead) => {
                if (err)
                    return sftp.status(reqId, statusCode(err));
                if (bytesRead === 0)
                    return sftp.status(reqId, ssh2.utils.sftp.STATUS_CODE.EOF);
...
async function startServerFtp(serverId) {
    const key = String(serverId);
    const serverRow = await (0, database_1.dbGet)('SELECT * FROM servers WHERE id = ?', [serverId]);
    if (!serverRow)
        throw new Error('Server not found');
    if (!serverRow.ftp_enabled)
        throw new Error('SFTP not enabled for this server');
    if (!serverRow.ftp_port)
        throw new Error('SFTP port not configured');
...
//# sourceMappingURL=ftpServer.js.map