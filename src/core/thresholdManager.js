...
function validateThresholds(metric, thresholds) {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (ACTION_SEVERITY[curr.action] < ACTION_SEVERITY[prev.action]) {
            errors.push(`Action "${curr.action}" at ${curr.val}${metaCfg.unit} has lower severity than ` +
...
function setThrottle(sid, pid, pct) {
    if (throttleApplied.get(sid) === pct)
        return;
    ...
    if (IS_WINDOWS) {
        ...
        child.on('error', () => (0, child_process_1.exec)(`renice -n ${pct >= 75 ? 0 : pct >= 40 ? 10 : 19} -p ${pid}`, () => { }));
    }
    logger.info(`[ThresholdManager] Throttle server ${sid} PID ${pid} @ ${pct}%`);
}
function clearThrottle(sid) {
    if (!throttleApplied.has(sid))
        return;
    ...
    if (IS_WINDOWS) {
        ...
        (0, child_process_1.exec)(`renice -n 0 -p ${pid}`, () => { });
    }
...
async function checkThresholds() {
    let servers;
    try {
        servers = await (0, database_1.dbAll)('SELECT id, name, ram_mb, threshold_rules FROM servers');
    }
    catch (err) {
        logger.error('[ThresholdManager] DB error:', err);
        return;
    }
