...
class WebhookManager {
    async create({ serverId, event, url }) {
        ...
        await (0, database_1.dbRun)(`INSERT INTO webhooks (server_id, event, url, active) VALUES (?,?,?,1)`, [serverId, event, url]);
        return { serverId, event, url };
    }
    async delete(id) {
        await (0, database_1.dbRun)(`DELETE FROM webhooks WHERE id = ?`, [id]);
    }
    async list(serverId) {
        return (0, database_1.dbAll)(`SELECT * FROM webhooks WHERE server_id = ?`, [serverId]);
    }
    async trigger(event, payload) {
        if (!ALLOWED_EVENTS.includes(event))
            return;
        const { serverId } = payload;
        const hooks = await (0, database_1.dbAll)(`SELECT id, url FROM webhooks WHERE server_id = ? AND event = ? AND active = 1`, [serverId, event]);
