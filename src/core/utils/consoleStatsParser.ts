...
function parsePlayersFromHistoryText(text) {
    try {
        ...
        let match, last = null;
        while ((match = regex.exec(source)) !== null) last = parseInt(match[1], 10);
...
export = { parseTpsFromHistoryText, parsePlayersFromHistoryText, parseTpsFromLine, isTpsConsoleNoise, isTpsUnknownCommand, isTpsCommandException, isStacktraceLine, isTpsErrorTrailer };
