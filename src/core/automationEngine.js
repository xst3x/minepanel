...
class AutomationEngine extends EventEmitter {
    ...
    async isAutomationActive(serverId) {
        ...
        if (this.activeCache.has(sid))
            return this.activeCache.get(sid);
        let active = false;
        try {
            const server = await (0, database_1.dbGet)('SELECT automation_enabled FROM servers WHERE id = ?', [serverId]);
            if (server && server.automation_enabled) {
                const row = await (0, database_1.dbGet)('SELECT 1 FROM automation_rules WHERE server_id = ? AND enabled = 1 AND script IS NOT NULL LIMIT 1', [serverId]);
                active = !!row;
            }
        }
        catch (err) {
    ...
    async triggerEvent(serverId, eventName, eventData = {}) {
        try {
            ...
            const rules = await (0, database_1.dbAll)('SELECT name, script FROM automation_rules WHERE server_id = ? AND enabled = 1 AND script IS NOT NULL', [serverId]);
            if (rules.length === 0)
                return;
