...
class PufferfishResolver {
    ...
    async resolveBuild(version, buildId = 'latest') {
        ...
        if (!line)
            throw new Error(`Unsupported Pufferfish version: ${version}`);
        ...
        if (!build)
            throw new Error(`No Pufferfish build found for ${version}`);
        const artifact = build.artifacts?.find(a => a.relativePath?.endsWith('.jar')) || build.artifacts?.[0];
        if (!artifact?.relativePath)
            throw new Error(`No Pufferfish jar artifact found for ${version} build ${build.number}`);
        return {
            type: TYPE,
            version,
