...
async function checkForUpdate(serverId) {
    ...
    await (0, database_1.dbRun)('UPDATE servers SET last_update_check = ? WHERE id = ?', [new Date().toISOString(), serverId]);
    setState(serverId, {
        status: 'idle',
        message: null,
...
async function runUpdate(serverId, opts = {}) {
    ...
    try {
        ...
        if (wasRunning) {
            ...
            await processManager.gracefulStop(String(serverId), 30000);
        }
        ...
        await (0, database_1.dbRun)('UPDATE servers SET version = ?, last_update_run = ? WHERE id = ?', [newMcVersion, new Date().toISOString(), serverId]);
        setState(serverId, {
            status: 'idle',
            message: null,
