...
function getEventsForScopes(scopes) {
    ...
    for (const scope of scopes) {
        ...
        if (scopeEvents)
            scopeEvents.forEach(e => events.add(e));
    }
    return events;
}
...
// ── Verify API key ──────────────────────────────────────────────────────
async function verifyApiKey(token, serverId, clientIp) {
    try {
        const prefix = token.length >= 20 ? token.substring(0, 20) : token;
        const key = await dbGet(`SELECT id, key_hash, scopes, expires_at, allowed_ips
...
function handleConnection(ws, req) {
    ...
    function attachListeners() {
        ...
        // Console listener
        const consoleListener = (srvId, output) => {
        ...
        // Status listener
        const statusListener = (srvId, status) => {
        ...
        const clearConsoleListener = (srvId) => {
        ...
        const automationLogListener = (srvId, logLine) => {
        ...
        const playerJoinListener = (srvId, player) => {
        ...
        const playerLeaveListener = (srvId, player) => {
        ...
        try {
            ...
            switch (parsed.type) {
                case 'subscribe': {
                    ...
                    const allowedEvents = getEventsForScopes(client.keyScopes);
                    events.forEach((e) => {
                        if (allowedEvents.has(e))
                            client.subscribed.add(e);
                    });
                ...
                case 'unsubscribe': {
                    const events = Array.isArray(parsed.events) ? parsed.events : [parsed.events];
                    events.forEach((e) => client.subscribed.delete(e));
...
//# sourceMappingURL=serverApiWebSocket.js.map