...
async function provisionIfNeeded(client, token, bot, serverId) {
    ...
    let integration = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE bot_id = ? AND server_id = ?', [bot.id, serverId]);
    if (!integration) {
        // Check if there is a legacy integration row with NULL/empty bot_id for this server
        const legacy = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE server_id = ? AND (bot_id IS NULL OR bot_id = 0)', [serverId]);
        if (legacy) {
            // Update it to belong to this bot
            await (0, database_1.dbRun)('UPDATE discord_integrations SET bot_id = ? WHERE id = ?', [bot.id, legacy.id]);
            integration = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE id = ?', [legacy.id]);
        }
        else {
            // Create stub row
            try {
                await (0, database_1.dbRun)(`INSERT OR IGNORE INTO discord_integrations (bot_id, server_id, guild_id, provisioned, bot_token_encrypted)
                     VALUES (?, ?, ?, 0, '')`, [bot.id, serverId, bot.guild_id]);
            }
            catch (_err) {
                await (0, database_1.dbRun)(`INSERT OR IGNORE INTO discord_integrations (bot_id, server_id, guild_id, provisioned)
                     VALUES (?, ?, ?, 0)`, [bot.id, serverId, bot.guild_id]);
            }
            integration = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE bot_id = ? AND server_id = ?', [botId, serverId]);
            if (!integration) {
                // Old schema with UNIQUE on server_id — row may exist for a different bot
                const existingRow = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE server_id = ?', [serverId]);
                if (existingRow) {
                    await (0, database_1.dbRun)('UPDATE discord_integrations SET bot_id = ? WHERE id = ?', [bot.id, existingRow.id]);
                    integration = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE id = ?', [existingRow.id]);
                }
            }
        }
    }
    if (!integration.provisioned) {
        ...
        await (0, database_1.dbRun)(`UPDATE discord_integrations SET
