...
async function startAll() {
    try {
        const bots = await (0, database_1.dbAll)('SELECT * FROM discord_bots WHERE enabled = 1');
        ...
        for (const bot of bots) {
            try {
                ...
                logger.error(`[Discord] Failed to start bot ${bot.id}:`, e.message);
            }
        }
    }
    catch (e) {
...
async function startBot(bot) {
    ...
    if (migrated) {
        try {
            ...
            await (0, database_1.dbRun)('UPDATE discord_bots SET bot_token_encrypted = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [reEncrypted, bot.id]);
    ...
    client.on(discord_js_1.Events.GuildCreate, async (guild) => {
        ...
        if (guild.id === bot.guild_id) {
            ...
            for (const { server_id } of botServers) {
                try {
                    await (0, database_1.dbRun)('UPDATE discord_integrations SET provisioned = 0 WHERE bot_id = ? AND server_id = ?', [bot.id, server_id]);
                    await provisionIfNeeded(client, token, bot, server_id);
