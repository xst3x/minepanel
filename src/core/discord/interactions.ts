...
import liveSessionMgr = require('./liveSessionManager')
 ...
 */
async function handleInteraction(interaction: any, botId: any, client: any) {
    ...
    try {
        await (command as any).execute(interaction, serverId);
    } catch (e) {
...
async function handleButton(interaction, serverId, integration, botId) {
    ...
    if (customId.startsWith('logs_')) {
        const logsCmd: any = commands.get('logs');
        if (logsCmd && logsCmd.handleButton) {
            if (!(await checkRole(interaction, integration, 'viewer'))) {
                return interaction.reply({ content: '❌ Missing required role.', ephemeral: true });
            }
            try { await (logsCmd as any).handleButton(interaction, serverId); } catch (e) {
    ...
    if (customId.startsWith('control_')) {
        ...
        if (action === 'refresh') {
            ...
            const cmd: any = commands.get(action);
            if (cmd) {
                try {
...
export = { handleInteraction, handleButton, checkRole, findServerMatch, buildServerListString, resolveServerId };
