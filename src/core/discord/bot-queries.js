...
async function listBots() {
    const bots = await (0, database_1.dbAll)('SELECT * FROM discord_bots ORDER BY created_at DESC');
    const result = [];
    for (const bot of bots) {
        const servers = await (0, database_1.dbAll)('SELECT server_id FROM discord_bot_servers WHERE bot_id = ?', [bot.id]);
...
async function getBot(botId) {
    ...
    if (!bot)
        return null;
    const servers = await (0, database_1.dbAll)('SELECT server_id FROM discord_bot_servers WHERE bot_id = ?', [botId]);
...
//# sourceMappingURL=bot-queries.js.map