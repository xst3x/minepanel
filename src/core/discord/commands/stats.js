...
async function fetchPlayerCount(serverId) {
    ...
    try {
        processManager.sendCommand(sid, 'list');
    }
    catch (_) {
        return null;
    }
    ...
    for (const line of newOutput.split('\n')) {
        ...
        if (m)
            return { playerCount: parseInt(m[1]), maxPlayers: parseInt(m[2]) };
    }
    return null;
}
        ...
        try {
            ...
            return interaction.editReply({
                embeds: [new EmbedBuilder()
                        ...
                        .addFields({ name: '🔧 CPU', value: `\`${cpuBar}\` **${cpu}%**`, inline: false }, { name: '💾 RAM', value: `\`${ramBar}\` **${ramMB}** / ${ramMax} MB (**${ramPct}%**)`, inline: false }, { name: 'Software', value: `${server.software} ${server.version}`, inline: true }, { name: 'Port', value: `${server.port}`, inline: true }, { name: 'CPU Cores', value: `${os.cpus().length}`, inline: true })
                        .setTimestamp()
