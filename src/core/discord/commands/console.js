...
module.exports = {
    ...
    async execute(interaction, serverId) {
        ...
        const buildEmbed = (isStopped = false, timedOut = false) => {
            ...
            let statusVal = '🔴 Live';
            if (isStopped)
                statusVal = timedOut ? '⏱ Auto-stopped (10 min)' : '⏹ Stopped';
            return new EmbedBuilder()
                .setTitle(isStopped
                ? `📟 Console — ${server.name}`
                ...
                .addFields({ name: 'Status', value: statusVal, inline: true }, { name: 'Lines', value: `${lines.length}`, inline: true }, { name: 'Session', value: elapsed, inline: true })
                .setTimestamp()
                .setFooter({ text: 'MinePanel • updates every ~4 s' });
        };
        const stopRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
    ...
    async _staticConsole(interaction, server, serverId) {
        ...
        return interaction.editReply({
            embeds: [new EmbedBuilder()
                    ...
                    .setDescription(`\`\`\`ansi\n${output}\n\`\`\``)
                    .setColor(0x1e293b)
                    .addFields({ name: 'Status', value: statusIcon, inline: true }, { name: 'Lines', value: `${lines.length}/${allLines.length}`, inline: true }, ...(filter ? [{ name: 'Filter', value: `\`${filter}\``, inline: true }] : []))
                    .setTimestamp()
