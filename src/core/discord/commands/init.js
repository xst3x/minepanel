...
module.exports = {
    ...
    async execute(interaction, serverId) {
        ...
        try {
            ...
            const integration = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE server_id = ?', [serverId]);
            if (!integration) {
            ...
            const resources = await provisionGuild(interaction.client, integration.guild_id, server.name, integration);
            // Persist the new IDs
            ...
            const updatedIntegration = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE server_id = ?', [serverId]);
            ...
            const successEmbed = new discord_js_1.EmbedBuilder()
                ...
                .setColor(0x22c55e)
                .addFields({ name: '🔑 Admin Role', value: `<@&${resources.adminRoleId}>`, inline: true }, { name: '🔑 Moderator Role', value: `<@&${resources.viewerRoleId}>`, inline: true }, { name: '\u200b', value: '\u200b', inline: true }, { name: '📟 Console', value: `<#${resources.consoleChannelId}>`, inline: true }, { name: '📊 Status', value: `<#${resources.statusChannelId}>`, inline: true }, { name: '⌨️ Commands', value: `<#${resources.logChannelId}>`, inline: true })
                .setTimestamp()
                .setFooter({ text: 'MinePanel' });
