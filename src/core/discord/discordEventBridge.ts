 ...
 */
import { EmbedBuilder, Events, PermissionFlagsBits, MessageFlags } from 'discord.js'
...
class DiscordEventBridge {
     ...
     */
    attach(bridgeKey: any, client: any, integration: any, realServerId?: any) {
        ...
        state.listeners.console = (emittedId: any, output: any) => {
        ...
        state.listeners.status = async (emittedId: any, status: any) => {
            if (emittedId.toString() !== serverId.toString()) return;
        ...
        state.listeners.clearConsole = async (emittedId: any) => {
            if (emittedId.toString() !== serverId.toString()) return;
            state.batchBuffer = '';
            if (state.batchTimer) {
        ...
        state.listeners.message = async (message: any) => {
