...
async function resolveServerId(interaction, botId) {
    ...
    for (const [key, integ] of integrationMap) {
        ...
        if (integ.guild_id === guildId) {
            if (interaction.channelId === integ.log_channel_id ||
...
async function handleButton(interaction, serverId, integration, botId) {
    ...
    if (customId.startsWith('live_console_stop_')) {
        ...
        if (session) {
            ...
            session.cleanup();
            try {
                await interaction.update({ embeds: [session.buildEmbed(true, false)], components: [] });
            }
            catch (_) { }
        }
        else {
    ...
    if (customId.startsWith('logs_')) {
        ...
        if (logsCmd && logsCmd.handleButton) {
            ...
            try {
                await logsCmd.handleButton(interaction, serverId);
            }
            catch (e) {
    ...
    if (customId.startsWith('control_')) {
        ...
        if (action === 'refresh') {
            try {
                ...
                const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`control_start_${serverId}`).setLabel('▶ Start').setStyle(ButtonStyle.Success).setDisabled(isOnline), new ButtonBuilder().setCustomId(`control_stop_${serverId}`).setLabel('⏹ Stop').setStyle(ButtonStyle.Danger).setDisabled(!isOnline), new ButtonBuilder().setCustomId(`control_restart_${serverId}`).setLabel('🔄 Restart').setStyle(ButtonStyle.Primary).setDisabled(!isOnline), new ButtonBuilder().setCustomId(`control_refresh_${serverId}`).setLabel('🔃 Refresh').setStyle(ButtonStyle.Secondary));
                await interaction.editReply({ embeds: [embed], components: [row] });
            }
            catch (e) {
                logger.error(`[Discord] Refresh error:`, e.message);
            }
        }
        else {
...
async function checkRole(interaction, integration, requiredRole) {
    ...
    if (adminRoleId && member.roles.cache.has(adminRoleId))
        return true;
    if (requiredRole === 'viewer' && viewerRoleId && member.roles.cache.has(viewerRoleId))
        return true;
    return false;
}
/**
