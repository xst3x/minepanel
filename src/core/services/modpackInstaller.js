...
const modrinthHttpModule = require("./modrinthHttp");
const { downloadFile } = modrinthHttpModule;
const modpackServiceModule = require("./modpackService");
const { resolveTargetVersion, loaderToSoftware, pickMcVersion } = modpackServiceModule;
const resolversModule = require("../resolvers");
const { resolveJar, downloadJar } = resolversModule;
const serverHelperModule = require("../serverHelper");
const { SERVERS_DIR, sanitizeDirName, ensureUniqueDirName } = serverHelperModule;
const serverHelpersModule = require("../../routes/modules/serverHelpers");
const { runForgeInstaller, runNeoForgeInstaller } = serverHelpersModule;
const processManager = require("../processManager");
const databaseModule = require("../../db/database");
const { dbRun } = databaseModule;
...
async function installMrpack(mrpackPath, serverDir) {
    ...
    for (const entry of zip.getEntries()) {
        ...
        if (!name.startsWith(overridesPrefix) || entry.isDirectory)
            continue;
...
async function installModpackFiles(versionData, serverDir) {
    ...
    if (!primaryFile?.url)
        throw new Error('Modpack version has no downloadable file');
...
//# sourceMappingURL=modpackInstaller.js.map