...
const { fetchJson, fetchText } = modrinthHttpModule;
...
const resolveSearchIndex = ({ query, category, sort }) => {
    if (sort && SORT_INDEX[sort])
        return SORT_INDEX[sort];
    if (query)
        return 'relevance';
 ...
 */
async function searchModpacks({ query = '', mcVersion = '', loader = '', category = 'popular', sort = '', limit = 15, offset = 0, }) {
...
async function getProject(projectId) {
    ...
    if (project.discord_url)
        links.push({ type: 'discord', url: project.discord_url });
    if (project.issues_url)
        links.push({ type: 'issues', url: project.issues_url });
    if (project.source_url)
        links.push({ type: 'source', url: project.source_url });
    if (project.wiki_url)
        links.push({ type: 'wiki', url: project.wiki_url });
...
//# sourceMappingURL=modpackService.js.map