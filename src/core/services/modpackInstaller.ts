...
import modrinthHttpModule = require('./modrinthHttp')
const { downloadFile } = modrinthHttpModule;
import modpackServiceModule = require('./modpackService')
const { resolveTargetVersion, loaderToSoftware, pickMcVersion } = modpackServiceModule;
import resolversModule = require('../resolvers')
const { resolveJar, downloadJar } = resolversModule;
import serverHelperModule = require('../serverHelper')
const { SERVERS_DIR, sanitizeDirName, ensureUniqueDirName } = serverHelperModule;
import serverHelpersModule = require('../../routes/modules/serverHelpers')
...
async function createModpackServer({ name, ram_mb, port, projectId, versionId, userId }) {
    const versionData = await resolveTargetVersion(projectId as any, { versionId });
    ...
    const mcVersion = pickMcVersion(versionData, null as any);
