...
const getUser = () => {
    if (!_User)
        _User = require('../db/database').User;
    return _User;
};
...
const generateToken = (user) => {
    return jwt.sign({ id: user.id, username: user.username, role: user.role, iat: Math.floor(Date.now() / 1000) }, SECRET_KEY, { expiresIn: process.env.JWT_EXPIRES_IN || '24h' });
};
...
const authenticateToken = (req, res, next) => {
    ...
    if (!token)
        return sendError(res, E.AUTH_UNAUTHORIZED, 401);
