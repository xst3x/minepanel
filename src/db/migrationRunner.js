...
const runMigrations = async (dbRun, dbGet, dbAll) => {
    ...
    for (const file of files) {
        ...
        const already = await dbGet('SELECT version FROM _migrations WHERE version = ?', [migration.version]);
        if (already)
        ...
        try {
            ...
            await dbRun('INSERT OR IGNORE INTO _migrations (version, description) VALUES (?, ?)', [migration.version, migration.description || file]);
