...
import ServerStats = require('./models/ServerStats')
import Rank = require('./models/Rank')
import UserServerPermission = require('./models/UserServerPermission')
import UserServerRank = require('./models/UserServerRank')
import Setting = require('./models/Setting')
import AccountCreationToken = require('./models/AccountCreationToken')
import DiscordBot = require('./models/DiscordBot')
import DiscordBotServer = require('./models/DiscordBotServer')
import DiscordIntegration = require('./models/DiscordIntegration')
import UserCustomAccent = require('./models/UserCustomAccent')
import Webhook = require('./models/Webhook')
import AuditLog = require('./models/AuditLog')
...
// Promise wrappers
const dbRun = (sql: string, params: any[] = []): Promise<any> => new Promise((resolve, reject) => {
    db.run(sql, params, function(this: any, err: Error|null) { err ? reject(err) : resolve(this); });
});
const dbGet = (sql: string, params: any[] = []): Promise<any> => new Promise((resolve, reject) => {
    db.get(sql, params, (err: Error|null, row: any) => { err ? reject(err) : resolve(row); });
});
const dbAll = (sql: string, params: any[] = []): Promise<any[]> => new Promise((resolve, reject) => {
    db.all(sql, params, (err: Error|null, rows: any[]) => { err ? reject(err) : resolve(rows || []); });
});

// ── Integrity Check ───────────────────────────────────────────────────────────
const checkIntegrity = (): Promise<{ok: boolean, errors: string[]}> => new Promise((resolve, reject) => {
...
const backupDatabase = async () => {
    ...
    const integrityOk = await new Promise<any>((resolve, reject) => {
...
const ensureAdminAccount = async () => {
    ...
    const password = Array.from(bytes, (b: any) => chars[b % chars.length]).join('');
...
export { db, dbRun, dbGet, dbAll, initDb, checkIntegrity, backupDatabase, listBackups, PREMADE_RANKS };
export { sequelize, User, Server, ServerStats, Rank, UserServerPermission, UserServerRank, Setting };
export { AccountCreationToken, DiscordBot, DiscordBotServer, DiscordIntegration, UserCustomAccent, Webhook, AuditLog };
