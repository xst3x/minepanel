...
function hasScope(keyScopes: string[], requiredScope: string): boolean {
  if (keyScopes.includes('server.everything')) return true
  ...
  return async (req: any, res: any, next: any) => {
    try {
      ...
      const keys = await dbGet(
        `SELECT id, key_hash, key_prefix, scopes, expires_at, allowed_ips
      ...
      // Parse scopes
      let keyScopes: string[] = ['server.read']
      try {
        keyScopes = JSON.parse(keys.scopes || '["server.read"]')
        if (!Array.isArray(keyScopes)) keyScopes = ['server.read']
      } catch { /* use default */ }
...
export = { apiKeyAuth, ALL_SCOPES, hasScope }
