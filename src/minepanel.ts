    ...
    ws.on('message', async (message) => {
        try {
                ...
                if (parsed.type === 'auth' && parsed.token) {
                    jwt.verify(parsed.token, SECRET_KEY, async (err, user) => {
                        ...
                        try {
                            ...
                            automationEngine.on('log', automationLogListener);                                    statsInterval = setInterval(async () => {
            ...
            if (parsed.type === 'chat') {
                ...
                if (text) processManager.sendCommand(serverId, `/say ${text}`);
            }
        } catch (e) {}
    });
...
}).catch(err => { console.error('Failed to initialize database:', err); process.exit(1); });    module.exports = { app, server };
