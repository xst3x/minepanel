...
async function readPlayerVitals(datPath) {
    try {
        ...
        const readName = () => {
            ...
            i += 2;
            const name = buf.slice(i, i + len).toString('utf8');
            i += len;
            return name;
        };
        const skipPayload = (type) => {
            ...
            else if (type === TAG_INT_ARR) {
                const len = buf.readInt32BE(i);
                i += 4 + len * 4;
            }
            else if (type === TAG_LONG_ARR) {
                const len = buf.readInt32BE(i);
                i += 4 + len * 8;
            }
        };
    ...
    try {
        ...
        if (UUID_FILE_RE.test(path.basename(statsFile))) {
            try {
                ...
                if (e.code !== 'ENOENT')
                    logger.warn(`[playerRoutes] Failed to read player stats:`, e);
        ...
        if (fs.existsSync(datFile))
            vitals = await readPlayerVitals(datFile);
        else if (fs.existsSync(datFileAlt))
            vitals = await readPlayerVitals(datFileAlt);
        res.json({
            uuid,
...
router.post('/:uuid/command', authenticateToken, checkPermission('server.players.manage'), async (req, res) => {
    ...
    if (action === 'wipe') {
        try {
            ...
            if (fs.existsSync(pDat))
                fs.unlinkSync(pDat);
            if (fs.existsSync(pStats))
                fs.unlinkSync(pStats);
    ...
    switch (action) {
        case 'kick':
            command = `kick ${safeUsername} ${safeValue || 'Kicked by panel'}`;
            break;
        case 'ban':
            command = `ban ${safeUsername} ${safeValue || 'Banned by panel'}`;
            break;
        case 'pardon':
            command = `pardon ${safeUsername}`;
            break;
        // mute/unmute use the standard /mute & /unmute commands (requires a mute plugin or Adventure mode)
        case 'mute':
            command = `mute ${safeUsername} ${safeValue || ''}`;
            break;
        case 'unmute':
            command = `unmute ${safeUsername}`;
            break;
        case 'op':
            command = `op ${safeUsername}`;
            break;
        case 'deop':
            command = `deop ${safeUsername}`;
            break;
        case 'gamemode':
            if (!safeValue)
                return sendError(res, E.BAD_REQUEST, 400, 'Gamemode value required');
            command = `gamemode ${safeValue} ${safeUsername}`;
            break;
        case 'xp':
            if (!safeValue)
                return sendError(res, E.BAD_REQUEST, 400, 'XP value required');
            command = `xp add ${safeUsername} ${safeValue}`;
            break;
        case 'give':
            if (!safeValue)
                return sendError(res, E.BAD_REQUEST, 400, 'Item value required');
            command = `give ${safeUsername} ${safeValue}`;
            break;
        case 'effect':
            if (!safeValue)
                return sendError(res, E.BAD_REQUEST, 400, 'Effect value required');
            command = `effect give ${safeUsername} ${safeValue}`;
            break;
        case 'clear':
            command = `clear ${safeUsername}`;
            break;
        case 'teleport':
            if (!safeValue)
                return sendError(res, E.BAD_REQUEST, 400, 'Teleport destination required');
            command = `tp ${safeUsername} ${safeValue}`;
            break;
        case 'heal':
            command = `effect give ${safeUsername} minecraft:regeneration 3 255`;
            break;
        case 'feed':
            command = `effect give ${safeUsername} minecraft:saturation 3 255`;
            break;
        case 'starve':
            command = `effect give ${safeUsername} minecraft:hunger 30 255`;
            break;
        case 'kill':
            command = `kill ${safeUsername}`;
            break;
        default:
            return sendError(res, E.PLAYER_ACTION_INVALID, 400);
    }
...
router.get('/lists/:listName', authenticateToken, checkPermission('server.players.read'), async (req, res) => {
    ...
    if (!filename)
        return sendError(res, E.PLAYER_LIST_INVALID, 400);
    try {
...
router.post('/lists/:listName', authenticateToken, checkPermission('server.players.manage'), async (req, res) => {
    ...
    if (!target)
        return sendError(res, E.BAD_REQUEST, 400, 'Target name or IP is required');
    const filename = listFileMap[listName];
    if (!filename)
        return sendError(res, E.PLAYER_LIST_INVALID, 400);
    const server = await getServer(serverId);
    ...
    try {
        if (isOnline) {
            ...
            if (listName === 'whitelist')
                ...
                command = `ban ${target} ${reason || 'Banned by panel'}`;
            else if (listName === 'banned-ips')
                command = `ban-ip ${target} ${reason || 'Banned by panel'}`;
            ...
            if (listName === 'banned-ips') {
                if (list.some(item => item.ip === target))
                    return sendError(res, E.BAD_REQUEST, 400, 'IP is already banned');
                ...
                if (listName === 'whitelist')
                    ...
                    list.push({ uuid, name: target, level: level !== undefined ? parseInt(level) : 4, bypassesPlayerLimit: false });
                else if (listName === 'banned-players')
                    list.push({ uuid, name: target, created: new Date().toISOString().replace('T', ' ').substring(0, 19) + ' +0000', source: 'Admin', expires: 'forever', reason: reason || 'Banned by panel' });
            }
    ...
    try {
        if (isOnline) {
            ...
            if (listName === 'banned-ips')
                ...
                filteredList = list.filter(item => !(item.name && item.name.toLowerCase() === target.toLowerCase()) && item.uuid !== target);
