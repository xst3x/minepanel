...
router.get('/', authenticateToken, checkPermission('server.backups.read'), async (req: any, res) => {
    try {
...
router.post('/create', authenticateToken, checkPermission('server.backups.create'), async (req: any, res) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.get('/:filename/download', authenticateToken, checkPermission('server.backups.read'), async (req: any, res) => {
...
router.post('/:filename/delete', authenticateToken, checkPermission('server.backups.delete'), async (req: any, res) => {
    const { filename } = req.params;
    if (!filename.endsWith('.zip') || filename.includes('..') || filename.includes('/') || filename.includes('\\')) {
        return sendError(res, E.BACKUP_INVALID_FILENAME, 400);
    }
...
router.post('/:filename/restore', authenticateToken, checkPermission('server.backups.restore'), async (req: any, res) => {
    const { filename } = req.params;
    if (!filename.endsWith('.zip') || filename.includes('..') || filename.includes('/') || filename.includes('\\')) {
        return sendError(res, E.BACKUP_INVALID_FILENAME, 400);
    }
        ...
        for (const s of servers) {
            ...
            try {
                ...
                if (existing.length > 0) {
                    const latest = existing.map(f => ({ name: f, time: fs.statSync(path.join(backupsDir, f)).mtime }))        .sort((a: any, b: any) => b.time - a.time)[0];
