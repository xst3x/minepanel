...
function generateApiKey(serverId: number): string {
...
router.get('/', async (req: any, res: any) => {
  try {
    ...
    const keys = await dbAll(
      `SELECT id, name, key_prefix, scopes, allowed_ips, expires_at, last_used_at, is_revoked, created_by, created_at
       FROM server_api_keys WHERE server_id = ? ORDER BY created_at DESC`,
      [serverId]
    )
...
router.post('/', async (req: any, res: any) => {
  try {
    ...
    const keyScopes: string[] = Array.isArray(scopes) && scopes.length > 0
      ? scopes : ['server.read']
    ...
    const keyAllowedIps: string[] = Array.isArray(allowed_ips) ? allowed_ips : []

    const result = await dbRun(
      `INSERT INTO server_api_keys (server_id, name, key_hash, key_prefix, scopes, allowed_ips, expires_at, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        serverId,
        keyName,
...
router.patch('/:keyId', async (req: any, res: any) => {
  try {
    const { serverId, keyId } = req.params
    const { name, scopes, allowed_ips } = req.body

    const key = await dbGet(
      'SELECT id FROM server_api_keys WHERE id = ? AND server_id = ?',
      [keyId, serverId]
    )
    if (!key) return sendError(res, E.NOT_FOUND, 404)
    ...
    params.push(keyId)
    await dbRun(
      `UPDATE server_api_keys SET ${updates.join(', ')} WHERE id = ?`,
      params
    )
...
router.delete('/:keyId', async (req: any, res: any) => {
  try {
    const { serverId, keyId } = req.params
    const key = await dbGet(
      'SELECT id FROM server_api_keys WHERE id = ? AND server_id = ?',
...
router.get('/scopes', async (req: any, res: any) => {
