...
import resolversModule = require('../core/resolvers');

const router = express.Router({ mergeParams: true });

interface RepairActionLog {
    action: string;
    status: 'success' | 'skipped' | 'failed' | 'preview';
    message: string;
}

router.post('/:serverId/repair', authenticateToken, checkPermission('server.settings.write'), async (req: any, res) => {
            ...
            if (actions.includes('properties')) {
                ...
                if (fs.existsSync(propPath)) {
                    if (dryRun) {
                        ...
                        if (propMap.has('query.port')) propMap.set('query.port', String(server.port));
                        if (propMap.has('rcon.port')) propMap.set('rcon.port', String(server.port + 10));
