...
import executionManager = require('../core/executionManager')
...
// ─── GET /info ─────────────────────────────────────────────────────────
router.get('/info', apiKeyAuth('server.read'), async (req: any, res: any) => {
  try {
...
// ─── GET /performance ──────────────────────────────────────────────────
router.get('/performance', apiKeyAuth('server.performance.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /players ──────────────────────────────────────────────────────
router.get('/players', apiKeyAuth('server.players.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /players/count ────────────────────────────────────────────────
router.get('/players/count', apiKeyAuth('server.players.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /players/:uuid ────────────────────────────────────────────────
router.get('/players/:uuid', apiKeyAuth('server.players.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    const stats = await executionManager.getStats(server.id.toString()).catch(() => null)
    const rawPlayers = stats?.playersList || []
    const player = rawPlayers.find((p: any) => p.uuid === req.params.uuid || p.name === req.params.uuid)
    if (!player) return sendError(res, E.PLAYER_NOT_FOUND, 404)
...
// ─── GET /console ──────────────────────────────────────────────────────
router.get('/console', apiKeyAuth('server.console.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (search) {
      lines = lines.filter((l: string) => l.toLowerCase().includes(search))
    }
...
// ─── POST /console ─────────────────────────────────────────────────────
router.post('/console', apiKeyAuth('server.console.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
router.post('/:action(start|stop|restart|kill|send-sigint)', apiKeyAuth('server.power'), async (req: any, res: any) => {
  try {
    ...
    switch (action) {
      case 'start': {
        ...
        if (status === 'online') return sendError(res, E.SERVER_ALREADY_RUNNING, 400)
        const serverDir = getServerDir(server)
...
// ─── GET /files ────────────────────────────────────────────────────────
router.get('/files', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /files/content ───────────────────────────────────────────────
router.get('/files/content', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!absPath.startsWith(serverDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
...
// ─── POST /files/write ────────────────────────────────────────────────
router.post('/files/write', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── DELETE /files ────────────────────────────────────────────────────
router.delete('/files', apiKeyAuth('server.files.delete'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)

    const serverDir = getServerDir(server)
    const relativePath = req.query.path as string || req.body.path || ''
    const absPath = path.join(serverDir, relativePath)

    if (!absPath.startsWith(serverDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
    if (!fs.existsSync(absPath)) return sendError(res, E.FILE_NOT_FOUND, 404)
...
// ─── POST /files/rename ───────────────────────────────────────────────
router.post('/files/rename', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!fs.existsSync(absOld)) return sendError(res, E.FILE_NOT_FOUND, 404)
    if (fs.existsSync(absNew)) return sendError(res, E.FILE_ALREADY_EXISTS, 409, 'Target name already exists')
...
// ─── POST /files/move ─────────────────────────────────────────────────
router.post('/files/move', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!fs.existsSync(absSource)) return sendError(res, E.FILE_NOT_FOUND, 404)
    if (fs.existsSync(absDest)) return sendError(res, E.FILE_ALREADY_EXISTS, 409, 'Destination already exists')
...
// ─── POST /files/copy ─────────────────────────────────────────────────
router.post('/files/copy', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!fs.existsSync(absSource)) return sendError(res, E.FILE_NOT_FOUND, 404)
    if (absSource === absDest) return sendError(res, E.BAD_REQUEST, 400, 'Source and destination are the same')
...
// ─── POST /files/upload ───────────────────────────────────────────────
router.post('/files/upload', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!absDest.startsWith(serverDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
    ...
    if (req.body.content !== undefined) {
      ...
      if (!filePath.startsWith(serverDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
...
// ─── GET /backups ──────────────────────────────────────────────────────
router.get('/backups', apiKeyAuth('server.backups.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── POST /backups ─────────────────────────────────────────────────────
router.post('/backups', apiKeyAuth('server.backups.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
router.post('/backups/:id/restore', apiKeyAuth('server.backups.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── DELETE /backups/:id ──────────────────────────────────────────────
router.delete('/backups/:id', apiKeyAuth('server.backups.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /plugins ──────────────────────────────────────────────────────
router.get('/plugins', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /plugins/:name ────────────────────────────────────────────────
router.get('/plugins/:name', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!pluginPath.startsWith(pluginDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
    if (!fs.existsSync(pluginPath)) return sendError(res, E.PLUGIN_NOT_FOUND, 404)
...
router.post('/plugins/install', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!filename) {
      ...
      if (!filename.endsWith('.jar') && !filename.endsWith('.phar')) filename += '.jar'
    }

    const destPath = path.join(pluginDir, filename)
    if (!destPath.startsWith(pluginDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
...
router.post('/plugins/:name/enable', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (fs.existsSync(activePath)) return res.json({ success: true, message: 'Plugin already enabled' })
    if (!fs.existsSync(disabledPath)) return sendError(res, E.PLUGIN_NOT_FOUND, 404)
...
router.post('/plugins/:name/disable', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!fs.existsSync(activePath)) return sendError(res, E.PLUGIN_NOT_FOUND, 404)
...
// ─── DELETE /plugins/:name ─────────────────────────────────────────────
router.delete('/plugins/:name', apiKeyAuth('server.files.delete'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /mods ─────────────────────────────────────────────────────────
router.get('/mods', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /mods/:name ───────────────────────────────────────────────────
router.get('/mods/:name', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!modPath.startsWith(modDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
    if (!fs.existsSync(modPath)) return sendError(res, E.PLUGIN_NOT_FOUND, 404)
...
// ─── POST /mods/install ────────────────────────────────────────────────
router.post('/mods/install', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!destPath.startsWith(modDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
...
router.post('/mods/:name/enable', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (fs.existsSync(activePath)) return res.json({ success: true, message: 'Mod already enabled' })
    if (!fs.existsSync(disabledPath)) return sendError(res, E.PLUGIN_NOT_FOUND, 404)
...
router.post('/mods/:name/disable', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── DELETE /mods/:name ────────────────────────────────────────────────
router.delete('/mods/:name', apiKeyAuth('server.files.delete'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /worlds ───────────────────────────────────────────────────────
router.get('/worlds', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── POST /worlds/load ─────────────────────────────────────────────────
router.post('/worlds/load', apiKeyAuth('server.console.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)

    const worldName = req.body.name || req.body.world || ''
    if (!worldName) return sendError(res, E.BAD_REQUEST, 400, 'World name is required')
...
// ─── POST /worlds/unload ───────────────────────────────────────────────
router.post('/worlds/unload', apiKeyAuth('server.console.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── POST /worlds/delete ───────────────────────────────────────────────
router.post('/worlds/delete', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!worldPath.startsWith(serverDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
    if (!fs.existsSync(worldPath)) return sendError(res, E.FILE_NOT_FOUND, 404, 'World directory not found')
...
// ─── GET /datapacks ────────────────────────────────────────────────────
router.get('/datapacks', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (fs.existsSync(datapacksDir)) {
      ...
      for (const entry of entries) {
        if (entry.name.startsWith('.minepanel-metadata-') || entry.name.startsWith('.')) continue
...
router.post('/datapacks/install', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!url) return sendError(res, E.BAD_REQUEST, 400, 'Download URL is required')
    ...
    if (!destPath.startsWith(datapacksDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
...
router.delete('/datapacks/:name', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!filePath.startsWith(datapacksDir)) return sendError(res, E.FILE_ACCESS_DENIED, 403)
    if (!fs.existsSync(filePath)) return sendError(res, E.FILE_NOT_FOUND, 404)
...
router.post('/datapacks/:name/enable', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (fs.existsSync(activePath)) return res.json({ success: true, message: 'Datapack already enabled' })
    if (!fs.existsSync(disabledPath)) return sendError(res, E.FILE_NOT_FOUND, 404, 'Datapack not found in disabled folder')
...
router.post('/datapacks/:name/disable', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!fs.existsSync(activePath)) return sendError(res, E.FILE_NOT_FOUND, 404, 'Datapack not found')
...
// ─── GET /logs ─────────────────────────────────────────────────────────
router.get('/logs', apiKeyAuth('server.files.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /statistics ───────────────────────────────────────────────────
router.get('/statistics', apiKeyAuth('server.performance.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    const history = await dbAll(
      `SELECT * FROM server_stats WHERE server_id = ? AND created_at >= ? ORDER BY created_at ASC`,
      [server.id, since.toISOString()]
    )
...
// ─── GET /environment ──────────────────────────────────────────────────
router.get('/environment', apiKeyAuth('server.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /tasks ────────────────────────────────────────────────────────
router.get('/tasks', apiKeyAuth('server.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── POST /tasks ───────────────────────────────────────────────────────
router.post('/tasks', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (!name || !type) return sendError(res, E.BAD_REQUEST, 400, 'Name and type are required')
...
// ─── PATCH /tasks/:id ──────────────────────────────────────────────────
router.patch('/tasks/:id', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)

    const tasksFile = path.join(getServerDir(server), 'server-tasks.json')
    if (!fs.existsSync(tasksFile)) return sendError(res, E.NOT_FOUND, 404, 'No tasks found')
    ...
    if (idx === -1) return sendError(res, E.NOT_FOUND, 404, 'Task not found')

    const { name, type, command, cron, enabled, description } = req.body
    if (name !== undefined) tasks[idx].name = name
    if (type !== undefined) tasks[idx].type = type
    if (command !== undefined) tasks[idx].command = command
    if (cron !== undefined) tasks[idx].cron = cron
...
// ─── DELETE /tasks/:id ─────────────────────────────────────────────────
router.delete('/tasks/:id', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
// ─── GET /variables ────────────────────────────────────────────────────
router.get('/variables', apiKeyAuth('server.read'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
    ...
    if (fs.existsSync(varsFile)) {
      try {
        ...
        if (typeof variables !== 'object' || Array.isArray(variables)) variables = {}
      } catch { variables = {} }
    }
...
// ─── POST /variables ───────────────────────────────────────────────────
router.post('/variables', apiKeyAuth('server.files.write'), async (req: any, res: any) => {
  try {
    const server = await getServer(req.apiKey.serverId)
    if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404)
...
function getWorldSize(serverDir: string): string {
  if (!fs.existsSync(serverDir)) return '0 B'
  try {
    const entries = fs.readdirSync(serverDir, { withFileTypes: true })
    for (const entry of entries) {
      if (entry.isDirectory()) {
...
function getStorageUsage(serverDir: string): string {
  if (!fs.existsSync(serverDir)) return '0 B'
  try {
...
async function getMotd(serverDir: string): Promise<string> {
...
async function getJavaVersion(server: any): Promise<string> {
  try {
    const { execSync } = require('child_process')
    const javaPath = server.java_path || 'java'
