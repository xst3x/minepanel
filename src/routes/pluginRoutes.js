...
const getCompatibleLoaders = (software) => {
    ...
    const map = {
        ...
        spigot: ['spigot', 'bukkit'], bukkit: ['bukkit'], fabric: ['fabric'], quilt: ['quilt'],
...
const getInstalledItems = async (server, { includeModrinth = true } = {}) => {
    ...
    for (const f of files) {
        ...
        const item = { name: f, size: stats.size, modifiedAt: stats.mtime, sha1: sha1File(filePath), folder: folderName };
        if (includeModrinth) {
            try {
...
router.get('/search', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.json({ hits: filtered, offset: data.offset ?? offset, limit: data.limit ?? limit, totalHits: data.total_hits ?? filtered.length });
    } catch (e) {
        logger.error(`[pluginRoutes] Search error (Server: ${serverId}, User: ${req.user.id}, Query: ${query}):`, e);
...
router.get('/project/:projectId', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({ ...project, modrinthUrl: `https://modrinth.com/${project.project_type || 'plugin'}/${project.slug || project.id}` });
    } catch (e) {
        logger.error(`[pluginRoutes] Project detail error (Server: ${req.params.serverId}, Project: ${req.params.projectId}):`, e);
...
router.get('/project/:projectId/versions', authenticateToken, async (req, res) => {
    try {
        ...
        logger.error(`[pluginRoutes] Project versions error (Server: ${req.params.serverId}, Project: ${req.params.projectId}):`, e);
...
router.get('/installed', authenticateToken, checkPermission('server.files.read'), async (req, res) => {
    try {
        ...
        logger.error(`[pluginRoutes] List installed error (Server: ${req.params.serverId}, User: ${req.user.id}):`, e);
        return sendError(res, E.INTERNAL_ERROR, 500);
    }
});
...
router.post('/update-all', authenticateToken, checkPermission('server.plugins.install'), async (req, res) => {
    try {
        ...
        for (const item of installed) {
            if (!item.modrinth) { results.push({ filename: item.name, status: 'skipped', reason: 'Not recognized on Modrinth' }); continue; }
            if (processedProjects.has(item.modrinth.projectId)) { results.push({ filename: item.name, status: 'skipped', reason: 'Duplicate installed project' }); continue; }
            ...
            try {
                ...
                if (!latest || latest.id === item.modrinth.versionId) {
                    results.push({ filename: item.name, projectId: item.modrinth.projectId, versionId: item.modrinth.versionId, versionNumber: item.modrinth.versionNumber, status: 'current' });
                    continue;
                }
                if (!isVersionCompatible(latest, server.version, softwareLower)) {
                    results.push({ filename: item.name, projectId: item.modrinth.projectId, status: 'skipped', reason: 'Latest response was not compatible' });
                    continue;
                }
                const primaryFile = await installVersionData(server, latest, { replaceProjectId: latest.project_id, oldFilename: item.name });
                results.push({ filename: item.name, newFilename: primaryFile.filename, projectId: latest.project_id, oldVersionId: item.modrinth.versionId, newVersionId: latest.id, oldVersionNumber: item.modrinth.versionNumber, newVersionNumber: latest.version_number, status: 'updated' });
            } catch (e) { results.push({ filename: item.name, status: 'skipped', reason: e.message }); }
        }

        const updated = results.filter(r => r.status === 'updated').length;
        const current = results.filter(r => r.status === 'current').length;
        const skipped = results.filter(r => r.status === 'skipped').length;
        res.json({ message: `Update complete: ${updated} updated, ${current} already current, ${skipped} skipped.`, updated, current, skipped, results });
    } catch (e) {
        logger.error(`[pluginRoutes] Update all error (Server: ${req.params.serverId}, User: ${req.user.id}):`, e);
...
router.post('/uninstall', authenticateToken, checkPermission('server.plugins.install'), async (req, res) => {
    ...
    try {
        ...
        logger.error(`[pluginRoutes] Uninstall error (Server: ${req.params.serverId}, User: ${req.user.id}, File: ${filename}):`, e);
        return sendError(res, E.INTERNAL_ERROR, 500);
    }
});
    ...
    try {
        ...
        if (!targetVersionId) {
            ...
            if (!allowIncompatible && !(vd.game_versions && vd.game_versions.includes(server.version))) {
                return sendError(res, E.PLUGIN_INCOMPATIBLE, 400, `This version does not support Minecraft ${server.version}. Supported: ${(vd.game_versions || []).join(', ')}`);
            }
            if (!allowIncompatible && !compatible) {
                return sendError(res, E.PLUGIN_INCOMPATIBLE, 400, `This version does not support ${server.software}. Supported loaders: ${(vd.loaders || []).join(', ')}`);
            }
        }
        ...
        for (const dep of requiredDeps) {
            try {
                ...
                if (!depVersionId) {
                    ...
                    if (!compatDep) { logger.warn(`[pluginRoutes] No compatible dep version for ${dep.project_id}`); continue; }
        ...
        logger.error(`[pluginRoutes] Install error (Server: ${req.params.serverId}, User: ${req.user.id}, Project: ${projectId}):`, e);
