...
const isPluginBased = (software) => ['paper', 'purpur', 'spigot', 'bukkit', 'magma'].includes(software.toLowerCase());
const isModBased = (software) => ['fabric', 'quilt', 'forge', 'neoforge'].includes(software.toLowerCase());
...
const sha1File = (filePath) => crypto.createHash('sha1').update(fs.readFileSync(filePath)).digest('hex');
...
const isBedrock = (software) => ['bedrock', 'bedrock-preview', 'pocketmine', 'nukkitx', 'powernukkitx', 'waterdogpe'].includes(software.toLowerCase());
...
const getInstalledDatapacks = async (server) => {
    ...
    for (const entry of entries) {
        ...
        if (entry.name.startsWith('.minepanel-metadata-') && entry.name.endsWith('.json'))
            continue;
...
const isHangarCompatible = (software) => ['paper', 'purpur', 'spigot', 'bukkit', 'waterfall', 'velocity'].includes(software.toLowerCase());
...
const removeInstalledProjectFiles = async (server, projectId, keepFilename) => {
    ...
    for (const item of installed) {
        if (item.name === keepFilename)
            continue;
        if (item.modrinth?.projectId !== projectId)
            continue;
...
const installVersionData = async (server, versionData, { replaceProjectId = null, oldFilename = null } = {}) => {
    ...
    if (replaceProjectId)
        await removeInstalledProjectFiles(server, replaceProjectId, primaryFile.filename);
    return primaryFile;
};
const pickCompatibleVersion = (versions, serverVersion, softwareLower) => versions.filter(v => isVersionCompatible(v, serverVersion, softwareLower))[0] || null;
...
router.get('/hangar/search', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const params = new URLSearchParams({ q: String(query), limit: String(limit), offset: String(offset), sort });
...
router.post('/datapacks/install', authenticateToken, checkPermission('server.plugins.install'), async (req, res) => {
    ...
    try {
        ...
        if (!targetVersionId) {
            ...
            if (!compatible)
                return sendError(res, E.PLUGIN_INCOMPATIBLE, 404, `No compatible datapack version found for game version ${server.version}.`);
...
router.post('/datapacks/uninstall', authenticateToken, checkPermission('server.plugins.install'), async (req, res) => {
    ...
    if (!folderName)
        return sendError(res, E.BAD_REQUEST, 400, 'Name is required');
    try {
        const server = await getServer(req.params.serverId);
        if (!server)
            return sendError(res, E.SERVER_NOT_FOUND, 404);
    ...
    try {
        ...
        if (source === 'hangar') {
            ...
            if (versionId) {
                ...
                if (!targetVersion)
                    return sendError(res, E.PLUGIN_INCOMPATIBLE, 404, `No compatible version found for Minecraft ${server.version}`);
            }
        ...
        if (!targetVersionId) {
            ...
            if (!compatible)
                return sendError(res, E.PLUGIN_INCOMPATIBLE, 404, `No compatible version found for ${server.software} ${server.version}.`);
            targetVersionId = compatible.id;
        }
        else {
...
router.post('/uninstall', authenticateToken, checkPermission('server.plugins.install'), async (req, res) => {
    ...
    if (!filename || !filename.endsWith('.jar'))
        return sendError(res, E.PLUGIN_INVALID_FILENAME, 400);
    try {
        const server = await getServer(req.params.serverId);
        if (!server)
            return sendError(res, E.SERVER_NOT_FOUND, 404);
    ...
    try {
        ...
        for (const item of installed) {
            ...
            if (item.modrinth) {
                ...
                try {
                    const latest = await fetchJson(`https://api.modrinth.com/v2/version_file/${item.sha1}/update?algorithm=sha1`, { method: 'POST', body: { loaders, game_versions: [server.version] } });
            ...
            try {
                ...
                const searchData = await fetchJson(`https://hangar.papermc.io/api/v1/projects?q=${encodeURIComponent(guess)}&limit=5&offset=0&sort=-downloads`);
                const match = (searchData.result || []).find(p => (p.name || '').toLowerCase() === guess.toLowerCase() ||
                ...
                const versionsData = await fetchJson(`https://hangar.papermc.io/api/v1/projects/${encodeURIComponent(owner)}/${encodeURIComponent(slug)}/versions?limit=10&offset=0`);
