...
const ftpServerModule = require("../../core/ftpServer");
const { startServerFtp, stopServerFtp, isServerFtpRunning, storePasswordCache, getPasswordCache } = ftpServerModule;
...
router.post('/:serverId/ftp/config', authenticateToken, checkPermission('server.ftp.manage'), validate(V.ftpConfig), async (req, res) => {
    try {
        ...
        if (!username || !port)
            return sendError(res, E.BAD_REQUEST, 400, 'username and port are required');
        ...
        if (conflict)
            return sendError(res, E.FTP_PORT_TAKEN, 400);
...
//# sourceMappingURL=serverFtpRoutes.js.map