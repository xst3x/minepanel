...
import serverHelpersModule = require('./serverHelpers')
const { getStartInfo } = serverHelpersModule;

const router = express.Router();

// ─── Start server ──────────────────────────────────────────────────────────
router.post('/:serverId/start', authenticateToken, checkPermission('server.start'), async (req: any, res) => {
...
router.post('/:serverId/stop', authenticateToken, checkPermission('server.stop'), async (req: any, res) => {
    const serverId = req.params.serverId;
    try {
        const server = await getServer(serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
// ─── Restart server ────────────────────────────────────────────────────────
router.post('/:serverId/restart', authenticateToken, checkPermission('server.restart'), async (req: any, res) => {
    const serverId = req.params.serverId;
    try {
        const server = await getServer(serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);

        const restartInfo: any = getStartInfo(server);
...
router.post('/:serverId/kill', authenticateToken, checkPermission('server.kill'), async (req: any, res) => {
    const serverId = req.params.serverId;
    try {
        const server = await getServer(serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.post('/:serverId/clear-console', authenticateToken, checkPermission('server.console.write'), async (req: any, res) => {
    const serverId = req.params.serverId;
    try {
        const server = await getServer(serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.post('/:serverId/command', authenticateToken, checkPermission('server.console.write'), async (req: any, res) => {
    try {
