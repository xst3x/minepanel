...
import ftpServerModule = require('../core/ftpServer')
...
function getCpuTemperature(): Promise<any> {
...
router.post('/settings', authenticateToken, validate(V.panelSettings), async (req, res) => {
    ...
    try {
        ...
        if (updated.ftpEnabled === true) {
            if (!ftpRunning) {
                try { await initFtpServer(); } catch (ftpErr) {
...
router.get('/detect-java', authenticateToken, async (req, res) => {
    ...
    const probeJava = (javaPath: string): Promise<any> => new Promise<any>((resolve) => {
...
function checkPortFree(port: number): Promise<any> {
