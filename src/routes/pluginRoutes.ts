...
const fetchJson = (url: string, options: any = {}) => new Promise<any>((resolve, reject) => {
...
const downloadFile = (url: string, dest: string) => new Promise<void>((resolve, reject) => {
...
const getInstalledItems = async (server) => {
    ...
    for (const f of files) {
        ...
        const item: any = { name: f, size: stats.size, modifiedAt: stats.mtime, sha1, folder: folderName };
...
router.get('/modrinth/search', authenticateToken, async (req: any, res) => {
...
router.get('/modrinth/project/:projectId', authenticateToken, async (req: any, res) => {
    try {
...
router.get('/modrinth/project/:projectId/versions', authenticateToken, async (req: any, res) => {
    try {
...
router.get('/hangar/search', authenticateToken, async (req: any, res) => {
    const { serverId } = req.params;
    const query = req.query.q || '';
...
router.get('/hangar/project/:owner/:slug', authenticateToken, async (req: any, res) => {
    try {
...
router.get('/hangar/project/:owner/:slug/versions', authenticateToken, async (req: any, res) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
        const { owner, slug } = req.params;
...
router.get('/datapacks/installed', authenticateToken, checkPermission('server.files.read'), async (req: any, res) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.get('/datapacks/search', authenticateToken, async (req: any, res) => {
    const { serverId } = req.params;
    const query = req.query.q || '';
    const limit = Math.min(Math.max(parseInt(req.query.limit, 10) || 24, 1), 100);
    const offset = Math.max(parseInt(req.query.offset, 10) || 0, 0);
...
router.get('/datapacks/project/:projectId', authenticateToken, async (req: any, res) => {
    try {
        const project = await fetchJson(`https://api.modrinth.com/v2/project/${encodeURIComponent(req.params.projectId)}`);
...
router.get('/datapacks/project/:projectId/versions', authenticateToken, async (req: any, res) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.post('/datapacks/install', authenticateToken, checkPermission('server.plugins.install'), async (req: any, res) => {
...
router.post('/datapacks/uninstall', authenticateToken, checkPermission('server.plugins.install'), async (req: any, res) => {
...
router.get('/installed', authenticateToken, checkPermission('server.files.read'), async (req: any, res) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.post('/install', authenticateToken, checkPermission('server.plugins.install'), async (req: any, res) => {
...
router.post('/uninstall', authenticateToken, checkPermission('server.plugins.install'), async (req: any, res) => {
...
router.post('/update-all', authenticateToken, checkPermission('server.plugins.install'), async (req: any, res) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
        const softwareLower = server.software.toLowerCase();
