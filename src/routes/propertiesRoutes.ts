...
router.get('/', authenticateToken, checkPermission('server.files.read'), async (req: any, res: any) => {
    try {
...
router.post('/', authenticateToken, checkPermission('server.files.write'), async (req: any, res: any) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
        const propsPath = path.join(getServerDir(server), 'server.properties');
...
router.get('/icon', authenticateToken, checkPermission('server.files.read'), async (req: any, res: any) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.post('/icon', authenticateToken, checkPermission('server.files.write'), iconUpload.single('icon'), async (req: any, res: any) => {
    try {
...
router.delete('/icon', authenticateToken, checkPermission('server.files.delete'), async (req: any, res: any) => {
    try {
        const server = await getServer(req.params.serverId);
        if (!server) return sendError(res, E.SERVER_NOT_FOUND, 404);
        const iconPath = path.join(getServerDir(server), 'server-icon.png');
