        ...
        if (platform === 'win32') {
            ...
            exec(cmd, { timeout: 2000 }, (error, stdout) => {
                ...
                if (lines.length > 1) {
                    ...
                    if (!isNaN(tempRaw))
                        return resolve((tempRaw - 2732) / 10);
                }
                resolve(null);
            });
        }
...
router.get('/detect-java', authenticateToken, async (req, res) => {
    ...
    if (isWin) {
        ...
        try {
            ...
            const out = execSync('wmic product where "Name like \'%Java%\' or Name like \'%JDK%\' or Name like \'%JRE%\'" get InstallLocation /value', { timeout: 4000, stdio: ['pipe', 'pipe', 'pipe'] }).toString();
