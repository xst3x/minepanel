...
import serverHelpersModule = require('./modules/serverHelpers');
const { getStartInfo } = serverHelpersModule;
import { ZipArchive as _ZipArchive } from 'archiver';
function archiver(_fmt: string, opts: any) { return new _ZipArchive(opts); }
import AdmZip = require('adm-zip');

const router = express.Router({ mergeParams: true });

// Memory cache for offline server worlds scanning
interface WorldCacheEntry {
    worlds: any[];
    timestamp: number;
}
const worldCache = new Map<number, WorldCacheEntry>();
...
async function scanForWorlds(dir: string, baseDir: string): Promise<any[]> {
    const list: any[] = [];
...
function getActiveWorldName(serverDir: string): string {
...
router.get('/:serverId/worlds', authenticateToken, checkPermission('server.files.read'), async (req: any, res) => {
...
router.post('/:serverId/worlds/clone', authenticateToken, checkPermission('server.files.write'), async (req: any, res) => {
    const serverId = req.params.serverId;
...
router.post('/:serverId/worlds/rename', authenticateToken, checkPermission('server.files.write'), async (req: any, res) => {
    const serverId = req.params.serverId;
    const { worldName, newName } = req.body;
...
router.post('/:serverId/worlds/delete', authenticateToken, checkPermission('server.files.write'), async (req: any, res) => {
    const serverId = req.params.serverId;
...
router.post('/:serverId/worlds/compress', authenticateToken, checkPermission('server.files.write'), async (req: any, res) => {
    const serverId = req.params.serverId;
    const { worldName } = req.body;

    if (!worldName) return sendError(res, E.BAD_REQUEST, 400, 'Parameter worldName is required.');
...
router.post('/:serverId/worlds/extract', authenticateToken, checkPermission('server.files.write'), async (req: any, res) => {
    const serverId = req.params.serverId;
    const { zipName } = req.body;

    if (!zipName) return sendError(res, E.BAD_REQUEST, 400, 'Parameter zipName is required.');
...
router.post('/:serverId/worlds/regenerate', authenticateToken, checkPermission('server.settings.write'), async (req: any, res) => {
    const serverId = req.params.serverId;
    const {
        difficulty,
        gamemode,
    ...
    try {
        ...
        if (fs.existsSync(propPath)) {
            ...
            if (difficulty) propMap.set('difficulty', difficulty);
            if (gamemode) propMap.set('gamemode', gamemode);
            if (levelSeed !== undefined) propMap.set('level-seed', levelSeed);
            if (generateStructures !== undefined) propMap.set('generate-structures', String(generateStructures));
            if (levelType) propMap.set('level-type', levelType);
            if (spawnMonsters !== undefined) propMap.set('spawn-monsters', String(spawnMonsters));
            if (spawnAnimals !== undefined) propMap.set('spawn-animals', String(spawnAnimals));
            if (spawnNpcs !== undefined) propMap.set('spawn-npcs', String(spawnNpcs));
        ...
        if (wasRunning) {
            const startInfo: any = getStartInfo(server);
