...
const { hashPassword, comparePassword, generateToken, invalidateToken, authenticateToken } = authModule;
...
const passwordValidatorModule = require("../core/utils/passwordValidator");
const { validatePasswordStrength } = passwordValidatorModule;
...
router.post('/login', loginLimiter, validate(V.login), async (req, res) => {
    ...
    if (!username || !password)
        return sendError(res, E.AUTH_INVALID_CREDENTIALS, 400);
    try {
        const user = await User.findOne({
            where: { username: { [sequelize_1.Op.eq]: username } }
        });
        if (!user) {
...
router.post('/register', validate(V.register), async (req, res) => {
    ...
    if (username.length < 3)
        return sendError(res, E.USER_USERNAME_TOO_SHORT, 400);
    if (username.length > 32)
        return sendError(res, E.USER_USERNAME_TOO_LONG, 400);
    if (password !== confirmPassword)
        return sendError(res, E.BAD_REQUEST, 400, "Passwords don't match");
    try {
        const settings = getSettings();
        const requireInviteToken = settings.requireInviteTokenToCreateAccount !== undefined
            ? settings.requireInviteTokenToCreateAccount : true;
        ...
        if (requireInviteToken) {
            if (!token)
                return sendError(res, E.AUTH_INVITE_TOKEN_REQUIRED, 400);
            dbToken = await AccountCreationToken.findOne({ where: { token } });
            if (!dbToken)
                return sendError(res, E.AUTH_INVITE_TOKEN_INVALID, 400);
            if (dbToken.used)
                return sendError(res, E.AUTH_INVITE_TOKEN_USED, 400);
            if (new Date(dbToken.expires_at).getTime() < Date.now())
                return sendError(res, E.AUTH_INVITE_TOKEN_EXPIRED, 400);
...
router.get('/2fa/setup', authenticateToken, async (req, res) => {
    try {
        ...
        await User.update({ totp_secret: secret, totp_verified: 0 }, { where: { id: req.user.id } });
...
router.post('/2fa/verify', authenticateToken, async (req, res) => {
    try {
        const { code } = req.body;
        if (!code)
            return sendError(res, E.BAD_REQUEST, 400, 'Code is required');
        const { verifySync } = require('otplib');
        ...
        if (!isValid)
            return sendError(res, E.AUTH_INVALID_CREDENTIALS, 400, 'Invalid code');
        ...
        await User.update({ totp_verified: 1, totp_backup_codes: JSON.stringify(backupCodes) }, { where: { id: req.user.id } });
...
router.post('/2fa/disable', authenticateToken, async (req, res) => {
    try {
        ...
        if (!password)
            return sendError(res, E.BAD_REQUEST, 400, 'Password is required');
        const user = await User.findByPk(req.user.id, {
        ...
        if (!match)
            return sendError(res, E.AUTH_INVALID_CREDENTIALS, 401);
        await User.update({ totp_enabled: 0, totp_verified: 0, totp_secret: null, totp_backup_codes: null }, { where: { id: req.user.id } });
...
router.post('/2fa/toggle', authenticateToken, async (req, res) => {
    try {
        ...
        if (typeof enable !== 'boolean')
            return sendError(res, E.BAD_REQUEST, 400, '"enable" must be a boolean');
        const user = await User.findByPk(req.user.id, {
        ...
        await User.update({ totp_enabled: enable ? 1 : 0 }, { where: { id: req.user.id } });
...
router.post('/2fa/regenerate-backup-codes', authenticateToken, async (req, res) => {
    try {
        const { code } = req.body;
        if (!code)
            return sendError(res, E.BAD_REQUEST, 400, 'Current TOTP code is required');
        const { verifySync } = require('otplib');
        const crypto = require('crypto');
        const user = await User.findByPk(req.user.id, {
        ...
        const isValid = verifySync({ strategy: 'totp', secret: user.totp_secret, token: String(code), epochTolerance: 1 });
        if (!isValid)
            return sendError(res, E.AUTH_INVALID_CREDENTIALS, 401, 'Invalid TOTP code');
        const backupCodes = Array.from({ length: 8 }, () => {
            const raw = crypto.randomBytes(5).toString('hex').toUpperCase();
            return raw.slice(0, 5) + '-' + raw.slice(5);
        });
        await User.update({ totp_backup_codes: JSON.stringify(backupCodes) }, { where: { id: req.user.id } });
...
router.post('/forgot-check', passwordResetLimiter, async (req, res) => {
    try {
        ...
        if (!username)
            return sendError(res, E.BAD_REQUEST, 400, 'Username is required');
        const user = await User.findOne({
            where: { username: { [sequelize_1.Op.eq]: username } },
...
router.post('/password-reset-with-totp', passwordResetLimiter, async (req, res) => {
    try {
        ...
        if (!username)
            return sendError(res, E.BAD_REQUEST, 400, 'Username is required');
        if (!newPassword)
            return sendError(res, E.BAD_REQUEST, 400, 'New password is required');
        const { validatePasswordStrength } = require('../core/utils/passwordValidator');
        const pwCheck = validatePasswordStrength(newPassword);
        if (!pwCheck.valid)
            return sendError(res, E.BAD_REQUEST, 400, pwCheck.error);
        const user = await User.findOne({
            where: { username: { [sequelize_1.Op.eq]: username } },
            attributes: ['id', 'username', 'totp_enabled', 'totp_secret', 'totp_backup_codes']
        });
        if (!user)
            return sendError(res, E.BAD_REQUEST, 404, 'Account not found');
        if (!user.totp_enabled || !user.totp_secret) {
        ...
        if (backupCode) {
            ...
            if (idx === -1)
                return sendError(res, E.AUTH_INVALID_CREDENTIALS, 401, 'Invalid backup code');
            codes.splice(idx, 1);
            await User.update({ totp_backup_codes: JSON.stringify(codes) }, { where: { id: user.id } });
        }
        else if (totpCode) {
            const { verifySync } = require('otplib');
            const verified = verifySync({ strategy: 'totp', secret: user.totp_secret, token: String(totpCode), epochTolerance: 1 });
            if (!verified)
                return sendError(res, E.AUTH_INVALID_CREDENTIALS, 401, 'Invalid authenticator code');
        }
        else {
        ...
        await User.update({ password: hashed, valid_tokens_from: now }, { where: { id: user.id } });
