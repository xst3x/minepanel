    ...
    try {
        ...
        for (const filePath of files) {
            try {
                ...
                const category = (meta as any).category || parts[0] || 'general';

                docs.push({
                    slug,
                    folder: parts.slice(0, -1).join('/') || '',
                    filename: parts[parts.length - 1],
                    title: (meta as any).title || parts[parts.length - 1],
                    category,
                    type: (meta as any).type || 'doc',
                    order: (meta as any).order ?? 999,
                    // category_order: from frontmatter, fallback to CATEGORY_ORDER map, fallback to 999
                    category_order: (meta as any).category_order ?? CATEGORY_ORDER[category] ?? 999,
                    content
                });
            } catch (fileErr) {
...
router.get('/:category/:page', authenticateToken, async (req: any, res: any) => {
    ...
    try {
        ...
        const { meta, content } = parseFrontmatter(raw);
        const parts = slug.split('/');
        const category_name = (meta as any).category || parts[0] || 'general';
        ...
        res.json({
            ...
            type: (meta as any).type || 'doc',
            order: (meta as any).order ?? 999,
            category_order: (meta as any).category_order ?? CATEGORY_ORDER[category_name] ?? 999,
            content
        });
    } catch (err) {
