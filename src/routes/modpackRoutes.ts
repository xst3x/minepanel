...
import modpackInstallerModule = require('../core/services/modpackInstaller')
...
router.get('/icon', async (req: any, res: any) => {
...
router.get('/categories', authenticateToken, (_req: any, res: any) => {
    res.json({ categories: MODPACK_CATEGORIES, loaders: LOADERS });
});

router.get('/game-versions', authenticateToken, async (_req: any, res: any) => {
    try {
...
router.get('/search', authenticateToken, async (req: any, res: any) => {
    ...
    const mcVersion = (req.query as any).mcVersion || '';
    const loader = (req.query as any).loader || '';
    const category = (req.query as any).category || 'popular';
    const sort = (req.query as any).sort || '';
    const limit = Math.min(Math.max(parseInt((req.query as any).limit, 10) || 15, 1), 100);
    const offset = Math.max(parseInt((req.query as any).offset, 10) || 0, 0);
 ...
 */
router.get('/version/:versionId/contents', authenticateToken, async (req: any, res: any) => {
    try {
...
router.get('/project/:projectId/versions', authenticateToken, async (req: any, res: any) => {
    try {
...
router.post('/create-server', authenticateToken, validate(V.createModpackServer), async (req: any, res: any) => {
