...
router.get('/list', authenticateToken, checkPermission('server.files.read'), async (req, res) => {
    try {
        ...
        try {
            ...
            return sendError(res, E.DIRECTORY_NOT_FOUND, 404);
        }
...
router.post('/create', authenticateToken, checkPermission('server.files.write'), validate(V.fileCreate), async (req, res) => {
    try {
        ...
        try {
            await fsp.access(safePath);
            return sendError(res, E.FILE_ALREADY_EXISTS, 409);
        }
        catch { }
...
router.get('/dl/:token', async (req, res) => {
    ...
    res.download(entry.file, entry.name, err => {
        ...
        if (err && !res.headersSent)
            sendError(res, E.INTERNAL_ERROR, 500);
    });
});
// ── Batch delete ──────────────────────────────────────────────────────────────
...
router.post('/extract', authenticateToken, checkPermission('server.files.write'), async (req, res) => {
    ...
    if (!archiveRelPath)
        return sendError(res, E.BAD_REQUEST, 400, 'path is required');
    try {
        const server = await getServer(req.params.serverId);
        if (!server)
            return sendError(res, E.SERVER_NOT_FOUND, 404);
        ...
        if (ext !== '.zip')
            return sendError(res, E.BAD_REQUEST, 400, 'Only .zip archives can be extracted');
...
router.get('/archive-tree', authenticateToken, checkPermission('server.files.read'), async (req, res) => {
    if (!req.query.path)
        return sendError(res, E.BAD_REQUEST, 400, 'path query param is required');
    try {
        const server = await getServer(req.params.serverId);
        if (!server)
            return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.post('/upload', authenticateToken, checkPermission('server.files.write'), (req, res, next) => {
    upload.single('file')(req, res, (err) => {
        ...
        if (!req.file)
            return sendError(res, E.BAD_REQUEST, 400, 'No file uploaded');
