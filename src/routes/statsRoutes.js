...
function toCSV(rows) {
    if (!rows.length)
        return 'collected_at,ram_bytes,cpu_percent,tps,players,disk_bytes\n';
    const header = 'collected_at,ram_bytes,cpu_percent,tps,players,disk_bytes';
    const lines = rows.map(r => `${r.collected_at},${r.ram_bytes},${r.cpu_percent},${r.tps ?? ''},${r.players},${r.disk_bytes ?? 0}`);
...
router.get('/', authenticateToken, checkPermission('server.stats.read'), async (req, res) => {
    try {
        ...
        const rows = await dbAll(`SELECT
             ...
             ORDER BY bucket_epoch ASC`, [bucketSeconds, bucketSeconds, bucketSeconds, bucketSeconds, serverId, minutes]);
...
router.get('/latest', authenticateToken, checkPermission('server.stats.read'), async (req, res) => {
    try {
        const { serverId } = req.params;
        const row = await dbGet(`SELECT collected_at, ram_bytes, cpu_percent, tps, players, disk_bytes
    ...
    try {
        ...
        const rows = await dbAll(`SELECT
             ...
             WHERE server_id = ?
               AND collected_at >= datetime('now', '-' || ? || ' minutes')
             GROUP BY bucket
             ORDER BY bucket ASC`, [serverId, minutes]);
        res.json({
            serverId,
            range: req.query.range || '1h',
...
router.get('/export', authenticateToken, checkPermission('server.stats.read'), async (req, res) => {
    try {
        const { serverId } = req.params;
        const { minutes } = parseRange(req.query);
        const rows = await dbAll(`SELECT collected_at, ram_bytes, cpu_percent, tps, players, disk_bytes
             FROM server_stats
             WHERE server_id = ?
               AND collected_at >= datetime('now', '-' || ? || ' minutes')
             ORDER BY collected_at ASC`, [serverId, minutes]);
...
configRouter.put('/', authenticateToken, async (req, res) => {
    try {
        ...
        if (retention_days !== undefined) {
            ...
            await dbRun(`INSERT INTO statistics_config (key, value, updated_at) VALUES ('retention_days', ?, CURRENT_TIMESTAMP)
                 ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`, [days.toString()]);
        }
        if (collection_interval_seconds !== undefined) {
            ...
            await dbRun(`INSERT INTO statistics_config (key, value, updated_at) VALUES ('collection_interval_seconds', ?, CURRENT_TIMESTAMP)
                 ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`, [secs.toString()]);
        }
