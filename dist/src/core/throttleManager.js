...
function getState(sid) {
    if (!throttleState.has(sid))
        throttleState.set(sid, { level: null });
...
function startWindowsThrottle(sid, pid, cpuLimitPct) {
    ...
    const iv = setInterval(() => {
        if (!suspended) {
            setTimeout(() => {
                if (!suspendJobs.has(sid))
                    return;
                (0, child_process_1.exec)(`powershell -NoProfile -NonInteractive -WindowStyle Hidden -Command "${psSuspend}"`, () => { });
                suspended = true;
            }, runMs);
        }
        else {
            (0, child_process_1.exec)(`powershell -NoProfile -NonInteractive -WindowStyle Hidden -Command "${psResume}"`, () => { });
            suspended = false;
        }
    }, cycleLenMs);
...
function startLinuxThrottle(sid, pid, cpuLimitPct) {
    ...
    const child = (0, child_process_1.spawn)('cpulimit', ['-p', String(pid), '-l', String(cpuLimitPct), '-z'], {
        stdio: 'ignore', detached: false
    });
    child.on('error', () => {
        // cpulimit not installed — use renice as fallback
        const niceVal = cpuLimitPct >= 75 ? 0 : cpuLimitPct >= 40 ? 10 : 19;
        (0, child_process_1.exec)(`renice -n ${niceVal} -p ${pid}`, (err) => {
            if (!err)
                logger.info(`[ThrottleManager] renice ${niceVal} applied to PID ${pid} (server ${sid})`);
        });
    });
...
function applyThrottle(sid, levelLabel, cpuLimitPct) {
    ...
    if (state.level === levelLabel)
        return; // already at this level, no change
...
function removeThrottle(sid) {
    ...
    if (IS_WINDOWS) {
        ...
        const child = processManager.processes.get(sid);
        if (child && child.pid)
            (0, child_process_1.exec)(`renice -n 0 -p ${child.pid}`, () => { });
    }
...
async function checkThrottles() {
    let servers;
    try {
        servers = await (0, database_1.dbAll)('SELECT id, name, ram_mb, throttle_config FROM servers');
    }
    catch (err) {
        logger.error('[ThrottleManager] DB error:', err);
        return;
    }
                ...
                getProcessRamBytes(child.pid).then(memBytes => {
                    ...
                    if (ramPct >= cfg.ramThresholdPercent) {
                        if (cfg.ramAction === 'stop' && !cd.ramStopping) {
                            ...
                            try {
                                processManager.sendCommand(sid, `say [MinePanel] High RAM usage! Server shutting down in 10 seconds.`);
                            }
                            catch (_) { }
                            setTimeout(() => { try {
                            ...
                            try {
                                processManager.sendCommand(sid, `say [MinePanel] Warning: High process RAM (${ramPct.toFixed(1)}%)!`);
                            }
                            catch (_) { }
                            cd.lastRamWarn = now;
                        }
        ...
        if (cfg.tempEnabled && cpuTemp !== null) {
            ...
            if (delta >= 0) {
                ...
                if (!cd.lastTempWarn || now - cd.lastTempWarn > WARN_COOLDOWN_MS) {
                    ...
                    try {
                        processManager.sendCommand(sid, `say [MinePanel] CPU temp ${cpuTemp.toFixed(1)}°C!${throttleInfo}`);
                    }
                    catch (_) { }
                    cd.lastTempWarn = now;
                }
                ...
                if (cfg.tempAction === 'stop' && chosen.label === 'heavy' && !cd.tempStopping) {
                    ...
                    try {
                        processManager.sendCommand(sid, `say [MinePanel] CRITICAL temperature! Server shutting down now!`);
                    }
                    catch (_) { }
                    setTimeout(() => { try {
                        processManager.gracefulStop(sid, 15000);
...
function stop() {
    ...
    for (const sid of throttleState.keys())
        removeThrottle(sid);
...
//# sourceMappingURL=throttleManager.js.map