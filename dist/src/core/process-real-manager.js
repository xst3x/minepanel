...
const processPersistence = require("./process-persistence");
...
class RealProcessManager extends EventEmitter {
                ...
                const placeholder = {
                    ...
                    kill: (signal) => {
                        try {
                            ...
                            logger.warn('[ProcessManager] Failed to set server state: ' + (e.message || e));
                        }
                    }
                };
    ...
    start(serverId, serverDir, javaArgs, jarFile, maxMemoryMb, customArgs = null, javaPath = 'java', spawnEnv = null, mode = 'java') {
        ...
        child.stdout?.on('data', (data) => {
            ...
            let output = raw;
            if (this._bedrockServers.has(serverId))
