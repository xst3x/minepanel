...
function parsePlayersFromHistory(serverId) {
    try {
        ...
        logger.warn('[StatsCollector] Failed to parse players from history: ' + (e.message || e));
    }
    return null;
}
// ── Collection cycle ──────────────────────────────────────────────────────────
async function collectStats() {
    try {
        ...
        for (const server of servers) {
            ...
            if (!online) {
                await dbRun(`INSERT INTO server_stats (server_id, ram_bytes, cpu_percent, tps, players, disk_bytes)
                     VALUES (?, 0, 0, null, 0, ?)`, [server.id, diskBytes]).catch(err => { logger.warn('[StatsCollector] Failed to insert offline stats: ' + (err.message || err)); });
                continue;
            }
            ...
            await dbRun(`INSERT INTO server_stats (server_id, ram_bytes, cpu_percent, tps, players, disk_bytes)
                 VALUES (?, ?, ?, ?, ?, ?)`, [server.id, ram, cpu, tps, players, diskBytes]).catch(err => {
...
async function pruneOldStats() {
    try {
        const cfg = await dbGet(`SELECT value FROM statistics_config WHERE key = 'retention_days'`).catch(() => null);
        const retentionDays = Math.max(1, parseInt(cfg?.value, 10) || DEFAULT_RETENTION_DAYS);
        const result = await dbRun(`DELETE FROM server_stats WHERE collected_at < datetime('now', '-' || ? || ' days')`, [retentionDays]);
