...
async function _processServer(server) {
    ...
    try {
        ...
        if (!result.compatible && !server.force_incompatible_updates) {
            logger.info(`[UpdateScheduler] Server ${sid} (${server.name}): ` +
        ...
        logger.error(`[UpdateScheduler] Error processing server ${sid} (${server.name}):`, err.message);
    }
}
/**
...
async function _tick() {
    ...
    _running = true;
    try {
        const servers = await dbAll('SELECT * FROM servers WHERE auto_update_software = 1');
