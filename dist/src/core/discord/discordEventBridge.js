...
class DiscordEventBridge {
    ...
    attach(bridgeKey, client, integration, realServerId) {
        ...
        state.listeners.console = (emittedId, output) => {
            if (emittedId.toString() !== serverId.toString())
                return;
        ...
        state.listeners.message = async (message) => {
            ...
            if (message.guildId !== integration.guild_id)
                return;
            ...
            if (!inCommandsChannel && !inConsoleChannel)
                return;
            ...
            const hasDiscordAdmin = member?.permissions?.has(discord_js_1.PermissionFlagsBits.Administrator);
        ...
        client.on(discord_js_1.Events.MessageCreate, state.listeners.message);
    ...
    detach(bridgeKey) {
        ...
        state.client.removeListener(discord_js_1.Events.MessageCreate, state.listeners.message);
        if (state.batchTimer) {
            clearTimeout(state.batchTimer);
            state.batchTimer = null;
        }
        ...
        try {
            ...
            if (state.statusMsgId) {
                try {
                    ...
                    await existing.edit({ embeds: [embed], flags: [discord_js_1.MessageFlags.SuppressNotifications] });
                    return;
                }
                catch (_) {
            ...
            try {
                ...
                if (ours) {
                    ...
                    await ours.edit({ embeds: [embed], flags: [discord_js_1.MessageFlags.SuppressNotifications] });
                    return;
                }
            }
            catch (_) { }
            // Send a fresh message and pin it
            const sent = await channel.send({ embeds: [embed], flags: [discord_js_1.MessageFlags.SuppressNotifications] });
    ...
    _buildStatusEmbed(isOnline, server) {
        ...
        if (server) {
            embed.addFields({ name: 'Status', value: isOnline ? '**Online ✅**' : '**Offline ⛔**', inline: true }, { name: 'Software', value: `${server.software} ${server.version}`, inline: true }, { name: 'Port', value: `\`${server.port}\``, inline: true }, { name: 'RAM', value: `${server.ram_mb} MB`, inline: true });
        }
        else {
    ...
    async _triggerReprovision(serverId, state) {
        if (state._reprovisioning)
            return;
        state._reprovisioning = true;
        try {
            const discordManager = require('./discordManager');
            const { dbRun } = require('../../db/database');
            await dbRun('UPDATE discord_integrations SET provisioned = 0 WHERE server_id = ?', [serverId]);
...
//# sourceMappingURL=discordEventBridge.js.map