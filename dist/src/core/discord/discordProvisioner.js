...
async function provisionGuild(client, guildId, serverName, integration = null) {
    ...
    if (!guild)
        throw new Error(`Bot is not in guild ${guildId}. Please invite the bot first.`);
    const botMember = await guild.members.fetchMe();
    for (const perm of [discord_js_1.PermissionFlagsBits.ManageRoles, discord_js_1.PermissionFlagsBits.ManageChannels]) {
    ...
    if (!category) {
        category = guild.channels.cache.find(c => c.type === discord_js_1.ChannelType.GuildCategory && c.name === categoryName);
    }
    if (!category) {
    ...
    try {
        const welcome = new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`**${serverName}** is now connected to Discord.\n\n` +
        ...
        await commandsChannel.send({ embeds: [welcome], flags: [discord_js_1.MessageFlags.SuppressNotifications] });
    }
    catch (_) { }
    return {
...
async function findOrCreateChannel(guild, channelId, name, category, botMember, opts) {
    ...
    if (!channel) {
        ...
        channel = guild.channels.cache.find(c => c.name === name && c.parentId === category.id);
    }
    if (channel)
        return channel;
    ...
    try {
        ...
        for (const channelId of [logChannelId, consoleChannelId, statusChannelId]) {
            ...
            try {
                ...
                if (ch)
                    await ch.delete('MinePanel: Integration removed');
            }
            catch (_) { }
        }
        ...
        if (categoryId) {
            try {
                ...
                if (cat)
                    await cat.delete('MinePanel: Integration removed');
            }
            catch (_) { }
        }
        else {
            ...
            try {
                for (const [, ch] of guild.channels.cache) {
                    if (ch.type === discord_js_1.ChannelType.GuildCategory &&
                        ch.name.includes('🎮') &&
        ...
        for (const roleId of [adminRoleId, viewerRoleId]) {
            ...
            try {
                ...
                if (role)
                    await role.delete('MinePanel: Integration removed');
            }
            catch (_) { }
        }
    }
...
//# sourceMappingURL=discordProvisioner.js.map