...
async function getStatusForServer(serverId) {
    const row = await (0, database_1.dbGet)('SELECT b.*, i.* FROM discord_bots b ' +
        ...
        'WHERE bs.server_id = ? LIMIT 1', [serverId]);
    if (!row)
        return null;
...
async function connect(serverId, botToken, guildId) {
    ...
    let bot = await (0, database_1.dbGet)('SELECT * FROM discord_bots WHERE bot_user_id = ? AND guild_id = ?', [botUser.id, guildId]);
    let botId;
    if (!bot) {
        ...
        await (0, database_1.dbRun)('UPDATE discord_bots SET enabled = 1 WHERE id = ?', [botId]);
...
async function disconnect(serverId) {
    ...
    const row = await (0, database_1.dbGet)('SELECT bot_id FROM discord_bot_servers WHERE server_id = ? LIMIT 1', [serverId]);
    if (!row)
        return { success: true };
    ...
    await (0, database_1.dbRun)('DELETE FROM discord_bot_servers WHERE bot_id = ? AND server_id = ?', [botId, serverId]);
    await (0, database_1.dbRun)('DELETE FROM discord_integrations WHERE bot_id = ? AND server_id = ?', [botId, serverId]);
    // If no servers left, delete the bot; otherwise restart
    const otherServers = await (0, database_1.dbAll)('SELECT 1 FROM discord_bot_servers WHERE bot_id = ?', [botId]);
...
async function toggleEnabled(serverId, enabled) {
    const row = await (0, database_1.dbGet)('SELECT bot_id FROM discord_bot_servers WHERE server_id = ? LIMIT 1', [serverId]);
    if (!row)
        throw new Error('No bot integration associated with this server.');
