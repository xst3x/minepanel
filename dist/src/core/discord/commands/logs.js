...
module.exports = {
    ...
    async execute(interaction, serverId) {
        ...
        try {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .setDescription(`\`\`\`\n${output || '(empty)'}\n\`\`\``)
                .setColor(0x1e293b)
                .addFields({ name: 'Page', value: `${currentPage}/${totalPages}`, inline: true }, { name: 'Total Lines', value: `${totalLines}`, inline: true }, ...(filter ? [{ name: 'Filter', value: `\`${filter}\``, inline: true }] : []))
                .setTimestamp()
    ...
    async handleButton(interaction, serverId) {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`\`\`\`\n${output || '(empty)'}\n\`\`\``)
            .setColor(0x1e293b)
            .addFields({ name: 'Page', value: `${newPage}/${totalPages}`, inline: true }, { name: 'Total Lines', value: `${totalLines}`, inline: true }, ...(filter ? [{ name: 'Filter', value: `\`${filter}\``, inline: true }] : []))
            .setTimestamp()
            .setFooter({ text: `${server.name} • MinePanel` });
        const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
            ...
            .setLabel('Next ▶')
            .setStyle(discord_js_1.ButtonStyle.Secondary)
            .setDisabled(newPage >= totalPages), new discord_js_1.ButtonBuilder()
