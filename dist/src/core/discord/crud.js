...
async function validateToken(botToken) {
    const tempClient = new discord_js_1.Client({ intents: [discord_js_1.GatewayIntentBits.Guilds] });
    try {
...
async function createBot(botToken, guildId, serverIds = []) {
    ...
    const result = await (0, database_1.dbRun)(`INSERT INTO discord_bots (bot_token_encrypted, guild_id, bot_user_id, bot_username, bot_avatar, enabled)
         VALUES (?, ?, ?, ?, ?, 1)`, [encryptedToken, guildId, botUser.id, botUser.username, botUser.avatar]);
    const botId = result.lastID;
    for (const serverId of serverIds) {
        await (0, database_1.dbRun)('INSERT OR IGNORE INTO discord_bot_servers (bot_id, server_id) VALUES (?, ?)', [botId, serverId]);
    }
    const bot = await (0, database_1.dbGet)('SELECT * FROM discord_bots WHERE id = ?', [botId]);
    await startBot(bot);
...
async function updateBot(botId, { botToken, guildId, serverIds } = {}) {
    ...
    const existing = await (0, database_1.dbGet)('SELECT * FROM discord_bots WHERE id = ?', [botId]);
    if (!existing)
    ...
    if (updates.length > 0) {
        ...
        params.push(botId);
        await (0, database_1.dbRun)(`UPDATE discord_bots SET ${updates.join(', ')} WHERE id = ?`, params);
    }
    if (serverIds !== undefined) {
        const oldAssignments = await (0, database_1.dbAll)('SELECT server_id FROM discord_bot_servers WHERE bot_id = ?', [botId]);
        ...
        if (removed.length > 0 && (!client || !client.isReady())) {
            try {
                ...
                tempClient = new discord_js_1.Client({ intents: [discord_js_1.GatewayIntentBits.Guilds] });
        ...
        for (const sid of removed) {
            ...
            if (!integ) {
                integ = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE bot_id = ? AND server_id = ?', [botId, sid]);
            }
            ...
            await (0, database_1.dbRun)('DELETE FROM discord_bot_servers WHERE bot_id = ? AND server_id = ?', [botId, sid]);
            await (0, database_1.dbRun)('DELETE FROM discord_integrations WHERE bot_id = ? AND server_id = ?', [botId, sid]);
        }
        if (tempClient) {
            try {
                tempClient.destroy();
        ...
        for (const sid of added) {
            await (0, database_1.dbRun)('INSERT OR IGNORE INTO discord_bot_servers (bot_id, server_id) VALUES (?, ?)', [botId, sid]);
        }
        ...
        if (client && client.isReady()) {
            let token;
            try {
                token = decrypt(existing.bot_token_encrypted);
            }
            catch (_) { }
            const freshBot = await (0, database_1.dbGet)('SELECT * FROM discord_bots WHERE id = ?', [botId]);
            const botServers = await (0, database_1.dbAll)('SELECT server_id FROM discord_bot_servers WHERE bot_id = ?', [botId]);
            for (const { server_id } of botServers) {
                try {
                    await (0, database_1.dbRun)('UPDATE discord_integrations SET provisioned = 0 WHERE bot_id = ? AND server_id = ?', [botId, server_id]);
    ...
    if (botToken || guildId) {
        const fresh = await (0, database_1.dbGet)('SELECT * FROM discord_bots WHERE id = ?', [botId]);
...
async function deleteBot(botId) {
    ...
    for (const { server_id } of botServers) {
        ...
        if (!integ) {
            integ = await (0, database_1.dbGet)('SELECT * FROM discord_integrations WHERE bot_id = ? AND server_id = ?', [botId, server_id]);
        }
        if (client && client.isReady() && integ && integ.provisioned) {
            try {
                await deprovisionGuild(client, integ.guild_id, {
    ...
    await stopBot(bid);
    await (0, database_1.dbRun)('DELETE FROM discord_bots WHERE id = ?', [botId]);
    await (0, database_1.dbRun)('DELETE FROM discord_bot_servers WHERE bot_id = ?', [botId]);
    await (0, database_1.dbRun)('DELETE FROM discord_integrations WHERE bot_id = ?', [botId]);
    return { success: true };
}
/**
...
async function toggleBot(botId, enabled) {
    await (0, database_1.dbRun)('UPDATE discord_bots SET enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [enabled ? 1 : 0, botId]);
    if (enabled) {
        const bot = await (0, database_1.dbGet)('SELECT * FROM discord_bots WHERE id = ?', [botId]);
        if (bot)
            await startBot(bot);
...
async function reprovisionServer(botId, serverId) {
    const bid = botId.toString();
    await (0, database_1.dbRun)('UPDATE discord_integrations SET provisioned = 0 WHERE bot_id = ? AND server_id = ?', [botId, serverId]);
    const bot = await (0, database_1.dbGet)('SELECT * FROM discord_bots WHERE id = ?', [botId]);
