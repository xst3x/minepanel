...
const { authenticateToken, hashPassword } = authModule;
const permissionsModule = require("../core/permissions");
const { checkGlobalPermission, getEffectivePermissions, AVAILABLE_PERMISSIONS, hasPermission } = permissionsModule;
...
router.post('/me/custom-accents', authenticateToken, validate(V.customAccentCreate), async (req, res) => {
    ...
    try {
        ...
        if (count >= 5)
            return sendError(res, E.BAD_REQUEST, 400, 'Maximum 5 custom colors allowed');
...
router.delete('/me/custom-accents/:colorId', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!row)
            return sendError(res, E.BAD_REQUEST, 404, 'Color not found');
        await row.destroy();
...
router.get('/:userId/permissions', authenticateToken, checkGlobalPermission('account.manage'), async (req, res) => {
    ...
    try {
        ...
        if (!user)
            return sendError(res, E.USER_NOT_FOUND, 404);
...
router.put('/:userId/permissions', authenticateToken, checkGlobalPermission('account.manage'), validate(V.userPermissions), async (req, res) => {
    ...
    try {
        ...
        if (!user)
            return sendError(res, E.USER_NOT_FOUND, 404);
        await User.update({ global_permissions: JSON.stringify(global) }, { where: { id: userId } });
...
router.put('/:userId/rank', authenticateToken, checkGlobalPermission('account.manage'), validate(V.setUserRank), async (req, res) => {
    ...
    try {
        ...
        if (rankId !== null) {
            ...
            if (!rank)
                return sendError(res, E.RANK_NOT_FOUND, 404);
        }
...
router.post('/change-name', authenticateToken, validate(V.changeName), async (req, res) => {
    ...
    if (!currentName || !newName || !confirmNewName)
        return sendError(res, E.BAD_REQUEST, 400, 'All fields are required');
    if (currentName !== req.user.username)
        return sendError(res, E.BAD_REQUEST, 400, 'Current name does not match logged-in user');
    if (newName !== confirmNewName)
        return sendError(res, E.BAD_REQUEST, 400, 'New name and confirm name do not match');
    if (newName.length < 3)
    ...
    try {
        ...
        if (e.name === 'SequelizeUniqueConstraintError')
            return sendError(res, E.USER_ALREADY_EXISTS, 409);
...
router.post('/change-password', authenticateToken, validate(V.changePassword), async (req, res) => {
    ...
    if (newPassword !== newPasswordConfirm)
        return sendError(res, E.BAD_REQUEST, 400, 'New passwords do not match');
    ...
    try {
        ...
        if (!match)
            return sendError(res, E.USER_PASSWORD_INCORRECT, 400);
...
router.post('/:userId/change-name', authenticateToken, checkGlobalPermission('account.manage'), validate(V.adminChangeName), async (req, res) => {
    ...
    if (!newName || !confirmNewName)
        return sendError(res, E.BAD_REQUEST, 400, 'All fields are required');
    if (newName !== confirmNewName)
        return sendError(res, E.BAD_REQUEST, 400, 'Names do not match');
    if (newName.length < 3)
        return sendError(res, E.USER_USERNAME_TOO_SHORT, 400);
    if (newName.length > 32)
        return sendError(res, E.USER_USERNAME_TOO_LONG, 400);
...
router.post('/:userId/reset-password', authenticateToken, checkGlobalPermission('account.manage'), validate(V.resetPassword), async (req, res) => {
    ...
    if (!newPassword || !confirmPassword)
        return sendError(res, E.BAD_REQUEST, 400, 'All fields are required');
    if (newPassword !== confirmPassword)
        return sendError(res, E.BAD_REQUEST, 400, 'Passwords do not match');
    // Validate password strength
    const passwordValidation = validatePasswordStrength(newPassword);
    if (!passwordValidation.valid) {
        return sendError(res, E.BAD_REQUEST, 400, passwordValidation.error);
    ...
    try {
        ...
        await User.update({ password: hashed, valid_tokens_from: now }, { where: { id: userId } });
...
router.patch('/:userId/toggle-disabled', authenticateToken, checkGlobalPermission('account.manage'), async (req, res) => {
    ...
    if (Number(userId) === Number(req.user.id))
        return sendError(res, E.USER_SELF_DISABLE, 400);
    try {
        const user = await User.findByPk(userId);
        if (!user)
            return sendError(res, E.USER_NOT_FOUND, 404);
...
router.post('/create', authenticateToken, checkGlobalPermission('account.manage'), validate(V.createUser), async (req, res) => {
    ...
    if (!username || !password)
        return sendError(res, E.BAD_REQUEST, 400, 'Username and password required');
    if (username.length < 3)
        return sendError(res, E.USER_USERNAME_TOO_SHORT, 400);
    if (username.length > 32)
        return sendError(res, E.USER_USERNAME_TOO_LONG, 400);
    if (password.length < 8)
        return sendError(res, E.USER_PASSWORD_TOO_SHORT, 400);
    try {
...
router.post('/me/avatar', authenticateToken, (req, res, next) => {
    avatarUpload.single('avatar')(req, res, (err) => {
        if (err)
            return sendError(res, E.BAD_REQUEST, 400, err.message);
        next();
    });
}, async (req, res) => {
    if (!req.file)
        return sendError(res, E.BAD_REQUEST, 400, 'No image file provided');
    try {
...
router.post('/me/password', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!match)
            return sendError(res, E.AUTH_INVALID_CREDENTIALS, 401, 'Current password is incorrect');
        const hashed = await hashPassword(newPassword);
        const now = Math.floor(Date.now() / 1000);
        await User.update({ password: hashed, valid_tokens_from: now }, { where: { id: req.user.id } });
