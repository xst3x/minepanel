...
router.get('/', authenticateToken, checkPermission('server.automation.read'), async (req, res) => {
    try {
        ...
        if (!server)
            return sendError(res, E.SERVER_NOT_FOUND, 404, 'Server not found');
...
router.post('/', authenticateToken, checkPermission('server.automation.write'), async (req, res) => {
    try {
        ...
        await dbRun(`INSERT INTO automation_rules (id, server_id, name, enabled, script)
             VALUES (?, ?, ?, ?, ?)`, [
            id,
            serverId,
            name.trim(),
...
router.get('/:ruleId', authenticateToken, checkPermission('server.automation.read'), async (req, res) => {
    try {
        ...
        if (!row)
            return sendError(res, E.SERVER_NOT_FOUND, 404, 'Rule not found');
...
router.put('/:ruleId', authenticateToken, checkPermission('server.automation.write'), async (req, res) => {
    try {
        ...
        await dbRun(`UPDATE automation_rules SET name = ?, script = ?, enabled = ? WHERE id = ?`, [newName, newScript, newEnabled, ruleId]);
