...
const { checkGlobalPermission } = permissionsModule;
...
router.get('/', requirePanelSettings, async (req, res) => {
    try {
        res.json(await discordManager.listBots());
    }
    catch (e) {
...
router.get('/servers', requirePanelSettings, async (req, res) => {
    try {
        res.json(await dbAll('SELECT id, name, software, version FROM servers ORDER BY name'));
    }
    catch (e) {
        return sendError(res, E.INTERNAL_ERROR, 500, e.message);
    }
...
router.post('/validate-token', requirePanelSettings, async (req, res) => {
    try {
        ...
        if (!botToken)
            return sendError(res, E.DISCORD_TOKEN_REQUIRED, 400);
...
router.post('/', requirePanelSettings, async (req, res) => {
    try {
        ...
        if (!/^\d{17,20}$/.test(guildId))
            return sendError(res, E.DISCORD_GUILD_INVALID, 400);
...
router.get('/:botId', requirePanelSettings, async (req, res) => {
    try {
        ...
        if (!bot)
            return sendError(res, E.BOT_NOT_FOUND, 404);
        res.json(bot);
    }
    catch (e) {
        return sendError(res, E.INTERNAL_ERROR, 500, e.message);
...
router.delete('/:botId', requirePanelSettings, async (req, res) => {
    try {
        res.json(await discordManager.deleteBot(req.params.botId));
    }
    catch (e) {
        return sendError(res, E.INTERNAL_ERROR, 500, e.message);
    }
...
router.post('/:botId/toggle', requirePanelSettings, async (req, res) => {
    try {
        ...
        if (typeof enabled !== 'boolean')
            return sendError(res, E.VALIDATION_ERROR, 400, '"enabled" (boolean) required');
...
//# sourceMappingURL=discordBotsRoutes.js.map