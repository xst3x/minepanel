...
router.post('/reorder', authenticateToken, async (req, res) => {
    ...
    if (!Array.isArray(order))
        return sendError(res, E.BAD_REQUEST, 400, 'order must be an array of rank IDs');
...
router.post('/create', authenticateToken, checkGlobalPermission('account.manage'), validate(V.createRank), async (req, res) => {
    ...
    if (!name)
        return sendError(res, E.RANK_FIELDS_INVALID, 400);
    if (name.length < 2 || name.length > 32)
        return sendError(res, E.BAD_REQUEST, 400, 'Name must be 2-32 characters');
    try {
        const result = await dbRun('INSERT INTO ranks (name, permissions, global_permissions, is_builtin, color) VALUES (?, ?, ?, 0, ?)', [name, '{}', '[]', color || '#3b82f6']);
...
router.put('/:rankId', authenticateToken, checkGlobalPermission('account.manage'), validate(V.updateRank), async (req, res) => {
    ...
    try {
        ...
        await dbRun('UPDATE ranks SET name = ?, color = ?, global_permissions = ?, permissions = ? WHERE id = ?', [nextName, color || rank.color, JSON.stringify(global), JSON.stringify(servers), rankId]);
...
router.post('/:rankId/update', authenticateToken, checkGlobalPermission('account.manage'), async (req, res) => {
    ...
    try {
        ...
        await dbRun('UPDATE ranks SET name = ?, color = ?, global_permissions = ?, permissions = ? WHERE id = ?', [nextName, nextColor, JSON.stringify(finalGlobal), JSON.stringify(finalServers), rankId]);
        res.json({ message: 'Rank updated' });
    }
    catch (e) {
...
router.post('/:rankId/delete', authenticateToken, checkGlobalPermission('account.manage'), async (req, res) => {
    ...
    try {
        ...
        if (!rank)
            return sendError(res, E.RANK_NOT_FOUND, 404);
        if (rank.is_builtin)
            return sendError(res, E.RANK_BUILTIN_PROTECTED, 403);
...
//# sourceMappingURL=rankRoutes.js.map