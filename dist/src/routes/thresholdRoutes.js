...
const { dbGet, dbRun } = databaseModule;
const logger = require("../core/utils/logger");
const { parseRules, validateThresholds, defaultRules, METRIC_CONFIG, ACTION_SEVERITY, } = require('../core/thresholdManager');
...
router.put('/', authenticateToken, checkPermission('server.properties.write'), async (req, res) => {
    ...
    try {
        ...
        const existing = parseRules((await dbGet('SELECT threshold_rules FROM servers WHERE id = ?', [serverId]))?.threshold_rules);
        ...
        await dbRun('UPDATE servers SET threshold_rules = ? WHERE id = ?', [JSON.stringify(merged), serverId]);
...
router.patch('/:metric/toggle', authenticateToken, checkPermission('server.properties.write'), validate(V.thresholdToggle), async (req, res) => {
    ...
    try {
        if (!(metric in METRIC_CONFIG))
            return sendError(res, E.BAD_REQUEST, 400, `Unknown metric "${metric}"`);
        const row = await dbGet('SELECT threshold_rules FROM servers WHERE id = ?', [serverId]);
        if (!row)
            return sendError(res, E.SERVER_NOT_FOUND, 404);
...
router.post('/:metric', authenticateToken, checkPermission('server.properties.write'), validate(V.thresholdAdd), async (req, res) => {
    ...
    try {
        ...
        if (!valid)
            return sendError(res, E.THRESHOLD_VALIDATION_FAILED, 400, errors);
...
//# sourceMappingURL=thresholdRoutes.js.map