            ...
            res.on('end', () => {
                if (res.statusCode >= 200 && res.statusCode < 300) {
                    try {
                        ...
                        reject(new Error('Invalid JSON from Poggit'));
                    }
                }
                else {
...
async function requirePocketMine(req, res, next) {
    try {
        ...
        if (server.software.toLowerCase() !== 'pocketmine') {
            return sendError(res, E.POCKETMINE_WRONG_SOFTWARE, 403, `This endpoint is only available for PocketMine-MP servers. ` +
        ...
        if (query) {
            ...
            filtered.sort((a, b) => {
                ...
                if (an === query && bn !== query)
                    return -1;
                if (bn === query && an !== query)
                    return 1;
                if (an.startsWith(query) && !bn.startsWith(query))
                    return -1;
                if (bn.startsWith(query) && !an.startsWith(query))
                    return 1;
...
router.get('/plugin/:name', authenticateToken, requirePocketMine, async (req, res) => {
    try {
        ...
        if (!plugin)
            return sendError(res, E.POCKETMINE_PLUGIN_NOT_FOUND, 404);
        res.json(plugin);
    }
    catch (e) {
...
router.post('/install', authenticateToken, checkPermission('server.plugins.install'), requirePocketMine, async (req, res) => {
    ...
    try {
        ...
        let release;
        if (version) {
            release = normalized.find(r => r.name.toLowerCase() === pluginName.toLowerCase() &&
                r.version === version);
        }
        else {
        ...
        if (!release)
            return sendError(res, E.POCKETMINE_PLUGIN_NOT_FOUND, 404, `Plugin "${pluginName}" not found on Poggit`);
        if (!release.artifact)
            return sendError(res, E.POCKETMINE_INSTALL_FAILED, 500, 'No download URL available for this plugin');
        const pluginsDir = path.join(getServerDir(server), PLUGINS_FOLDER);
        if (!fs.existsSync(pluginsDir))
        ...
        const existing = fs.readdirSync(pluginsDir).filter(f => f.endsWith('.phar') &&
            f.toLowerCase().startsWith(release.name.toLowerCase().replace(/[^a-zA-Z0-9.\-_]/g, '_')) &&
            f !== safeFilename);
        for (const old of existing) {
            try {
                fs.unlinkSync(path.join(pluginsDir, old));
            }
            catch (_) { }
        }
...
//# sourceMappingURL=pocketmineRoutes.js.map