    ...
    try {
        ...
        if (hasRunScript || hasArgsFile) {
            ...
            if (!forgeJar) {
                ...
                if (directForge)
                    forgeJar = path.join(serverDir, directForge);
            }
            if (forgeJar) {
        ...
        fs.appendFileSync(logFile, `[${new Date().toISOString()}] ERROR: ${errMsg}\n`);
        if (err.stdout)
            fs.appendFileSync(logFile, `STDOUT: ${err.stdout}\n`);
        if (err.stderr)
            fs.appendFileSync(logFile, `STDERR: ${err.stderr}\n`);
        logger.error(`[Forge] ${errMsg}`);
...
function findNeoForgeJarRecursive(dir) {
    try {
        ...
        for (const entry of entries) {
            ...
            else if (entry.name.match(/^neoforge-.*-server\.jar$/) ||
...
async function runNeoForgeInstaller(installerPath, serverDir, serverId) {
    ...
    try {
        ...
        if (!neoforgeJar) {
            ...
            if (direct)
                neoforgeJar = path.join(serverDir, direct);
        }
        if (!neoforgeJar) {
            const runBat = path.join(serverDir, process.platform === 'win32' ? 'run.bat' : 'run.sh');
            if (fs.existsSync(runBat)) {
