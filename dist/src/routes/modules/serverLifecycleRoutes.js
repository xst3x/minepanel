...
const { dbRun, dbGet } = databaseModule;
...
router.post('/:serverId/start', authenticateToken, checkPermission('server.start'), async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (isBedrock) {
                processManager.start(serverId.toString(), serverDir, [], startInfo.executable, server.ram_mb, [], startInfo.executable, startInfo.env, 'bedrock');
            }
            else if (isPocketMine) {
                processManager.start(serverId.toString(), serverDir, [], startInfo.jarFile, server.ram_mb, startInfo.customArgs, startInfo.executable, startInfo.env, 'pocketmine');
            }
            else {
...
router.post('/:serverId/restart', authenticateToken, checkPermission('server.restart'), async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            let result;
            if (isBedrock) {
                result = await processManager.restartGraceful(serverId.toString(), serverDir, [], restartInfo.executable, server.ram_mb, 15000, [], restartInfo.executable, restartInfo.env, 'bedrock');
            }
            else if (isPocketMine) {
                result = await processManager.restartGraceful(serverId.toString(), serverDir, [], restartInfo.jarFile, server.ram_mb, 15000, restartInfo.customArgs, restartInfo.executable, restartInfo.env, 'pocketmine');
            }
            else {
                const javaPath = await javaManager.getJavaPath(server.java_path);
                result = await processManager.restartGraceful(serverId.toString(), serverDir, [], jarFile, server.ram_mb, 15000, customArgs, javaPath);
            }
            if (!result.graceful) {
...
//# sourceMappingURL=serverLifecycleRoutes.js.map