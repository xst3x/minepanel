...
const { db, dbRun, dbGet, dbAll } = databaseModule;
...
const { resolveJar, downloadJar } = resolversModule;
...
const fsRetryModule = require("../../core/utils/fsRetry");
const { retryRename, retryDelete, retryUnlink, retryCopy } = fsRetryModule;
    ...
    try {
        ...
        if (user && user.role === 'admin') {
            ...
            const hasGlobalServerAccess = globalPerms.includes('*') || globalPerms.includes('root') ||
                globalPerms.some((p) => p.startsWith('server.'));
...
router.post('/create', authenticateToken, checkGlobalPermission('server.create'), validate(V.createServer), async (req, res) => {
    ...
    try {
        ...
        const result = await dbRun('INSERT INTO servers (uuid, name, software, version, ram_mb, port, owner_id, directory_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [uuid, name, software, version, ram_mb, port, userId, dirName]);
...
router.post('/:serverId/change-version', authenticateToken, checkPermission('server.properties.write'), validate(V.changeVersion), async (req, res) => {
    ...
    if (!version)
        return sendError(res, E.BAD_REQUEST, 400, 'Version required');
    try {
            ...
            if (oldType !== newType) {
                if (isModded(oldType) && !isModded(newType)) {
                    ...
                    if (fs.existsSync(modsPath)) {
                        try {
                            if (fs.existsSync(disabledMods))
                                await retryDelete(disabledMods);
                    ...
                    if (fs.existsSync(disabledPlugins) && !fs.existsSync(pluginsPath)) {
                        try {
                            await retryRename(disabledPlugins, pluginsPath);
                        }
                        catch (_) { }
                    }
                }
                if (isPluginBased(oldType) && !isPluginBased(newType) && isModded(newType)) {
                    ...
                    if (fs.existsSync(pluginsPath)) {
                        try {
                            if (fs.existsSync(disabledPlugins))
                                await retryDelete(disabledPlugins);
                    ...
                    if (fs.existsSync(disabledMods) && !fs.existsSync(modsPath)) {
                        try {
                            await retryRename(disabledMods, modsPath);
                        }
                        catch (_) { }
                    }
                }
...
router.post('/:serverId/backup-config', authenticateToken, checkPermission('server.backups.create'), validate(V.backupConfig), (req, res) => {
    ...
    db.run('UPDATE servers SET auto_backup = ?, backup_interval = ?, backup_includes = ? WHERE id = ?', [enabled ? 1 : 0, interval || 24, includes || 'all', serverId], function (err) {
        if (err) {
...
router.post('/import', authenticateToken, importUpload.single('archive'), async (req, res) => {
    ...
    if (!req.file)
        return sendError(res, E.BAD_REQUEST, 400, 'No archive uploaded');
    ...
    try {
        ...
        const result = await dbRun('INSERT INTO servers (uuid, name, software, version, ram_mb, port, owner_id, directory_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [uuid, name, software, version, ramNum, portNum, userId, dirName]);
        ...
        if (e.code === 'SQLITE_CONSTRAINT') {
            if (e.message.includes('servers.name'))
                return sendError(res, E.SERVER_NAME_TAKEN, 409);
            if (e.message.includes('servers.port'))
                return sendError(res, E.SERVER_PORT_TAKEN, 409);
        }
...
router.post('/:serverId/settings', authenticateToken, checkPermission('server.properties.write'), validate(V.serverSettings), async (req, res) => {
    ...
    try {
        ...
        if (!server)
            return sendError(res, E.SERVER_NOT_FOUND, 404);
        if (!name)
            return sendError(res, E.BAD_REQUEST, 400, 'Server name is required');
        const portNum = parseInt(port, 10);
        ...
        await dbRun(`UPDATE servers SET 
                name = ?, 
                port = ?, 
                ram_mb = ?, 
                java_path = ?, 
                ...
                autostart = ?,
                autostart_on_crash = ?
             WHERE id = ?`, [name, portNum, ramNum, javaPathStr, logRet, backupRet,
...
//# sourceMappingURL=serverManagementRoutes.js.map