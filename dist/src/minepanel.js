...
if (process.env.MINEPANEL_SERVER !== 'true' && process.env.NODE_ENV !== 'test') {
    ...
    wss.on('connection', (ws, req) => {
                    ...
                    if (parsed.type === 'auth' && parsed.token) {
                        jwt.verify(parsed.token, SECRET_KEY, async (err, user) => {
                            ...
                            try {
                                ...
                                consoleListener = (sid, output) => { if (sid === serverId)
                                    ws.send(JSON.stringify({ type: 'console', data: output })); };
                                statusListener = (sid, status) => { if (sid === serverId)
                                    ws.send(JSON.stringify({ type: 'status', data: status })); };
                                clearConsoleListener = (sid) => { if (sid === serverId)
        ...
        ws.on('close', () => {
            ...
            if (clearConsoleListener)
                processManager.removeListener('clear_console', clearConsoleListener);
            if (automationLogListener)
                automationEngine.removeListener('log', automationLogListener);
        });
    });
    ...
    initDb().then(async () => {
        ...
        try {
            ...
            logger.warn('[UpdateScheduler] Failed to start: ' + e.message);
        }
                ...
                setTimeout(async () => {
                    for (const srv of autostartServers) {
                        try {
                            ...
                            logger.error(`[Autostart] Failed to start server ${srv.id}: ${e.message}`);
                        }
                    }
                }, 3000);
            }
        ...
        global.changePortAndRestart = async (newPort) => {
            try {
                ...
                if (global.activeSockets) {
                    for (const s of global.activeSockets)
                        s.destroy();
